...
## Phase 11 — Deferred cleanups close-out (tagged: phase-11-complete)

Closes all 6 items from Known Deferred Cleanups across two sessions.

**Session 1** (tagged `phase-11-deferred-cleanups`, commit 640f86c):
...
**Session 2** (tagged `phase-11-complete`, commit af7f3a1):

- Task 3 (piConfig drift guard): closed administratively. Commit 5548dc9
  (`refactor(brand): auto-sync piConfig`) already implements Option B
  (collapse to one source of truth): root `package.json` is canonical,
  mirrors are auto-synced on `prebuild` via `scripts/sync-piconfig.mjs`,
  and `scripts/verify-piconfig-sync.mjs` blocks `prepublishOnly` on
  any drift. The originally-recommended Option A (CI parity check on
  prebuild) would be a no-op layered on top of the sync. See Phase 11
  Residue item #12 below for the full rationale.
- Task 4 (Loop24 Signal Theme generator): wrote
  `scripts/generate-theme-const.mjs` that emits
  `packages/pi-coding-agent/src/modes/interactive/theme/loop24-theme.generated.ts`
  from the canonical JSON at `src/resources/extensions/loop24/theme/loop24.json`.
  Wired into `prebuild`. `themes.ts` now imports the generated const
  rather than carrying a hand-maintained duplicate. JSON updated to
  promote `borderMuted` to a `vars` entry for structural parity with
  the prior TS form (resolved hex values unchanged).
...
### 12. piConfig drift guard — RESOLVED in commit 5548dc9 (administratively closed in Phase 11 Session 2)

`piConfig` is still triplicated across root `package.json`,
`packages/pi-coding-agent/package.json`, and `pkg/package.json` — the
three files can't be collapsed because each is read synchronously by
a different runtime context (loader-fast JSON.parse path, workspace
package resolution, and the `PI_PACKAGE_DIR=pkg/` shim respectively).

The drift problem itself is resolved: commit 5548dc9
(`refactor(brand): auto-sync piConfig — single source of truth`) made
root `package.json` the canonical source. `scripts/sync-piconfig.mjs`
overwrites the two mirrors on every `prebuild`, and
`scripts/verify-piconfig-sync.mjs` is wired into `prepublishOnly` to
hard-fail any drift before a publish escapes. The mirrors carry an
AUTO-SYNC `_comment` warning against hand-edits.

The Phase 11 plan's original recommendation was Option A (a CI parity
check on `prebuild`). That recommendation predates 5548dc9; layered
on top of the auto-sync it would be a no-op because the sync writes
first. The implemented design (auto-sync on prebuild + verify on
prepublishOnly) is strictly stronger than the originally proposed
parity check.

### 13. Loop24 Signal Theme JSON→TS generator — RESOLVED in Phase 11 Task 4

`loop24.json` is now the canonical source. The runtime const in
`themes.ts` is auto-generated by `scripts/generate-theme-const.mjs`
on every `prebuild`, emitting
`packages/pi-coding-agent/src/modes/interactive/theme/loop24-theme.generated.ts`.
`themes.ts` imports from the generated file rather than carrying a
hand-maintained duplicate. To rebrand: edit only the JSON.

A small JSON cleanup was bundled in: `borderMuted` was promoted from
3 inline `#2A2A2A` hex literals (in `colors`) to a single `vars`
entry, so the generated TS matches the prior hand-written const

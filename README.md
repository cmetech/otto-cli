...
Plans live in [`docs/superpowers/plans/`](docs/superpowers/plans/) — one per phase, executed by subagents. The `otto` extension source is in `src/resources/extensions/otto/`.
...
MIT — see [`LICENSE`](LICENSE). Inherited from upstream `otto-pi`; copyright Lex Christopherson 2026.

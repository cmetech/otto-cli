# OTTO Mascot — Text-to-Image Prompt

Use this prompt with a text-to-image tool that supports **image references**. Attach the OSCAR logo (the baby-blue cartoon robot with the TV-screen face, three red lights on top, and "AI" on the base) as the **style reference image**. The prompt below tells the model to match OSCAR's illustration DNA exactly and translate it onto an otto character.
...
The character is an OTTO (a cartoon sea otto), not a robot. It is the
...
- Where OSCAR has a square TV-screen face, OTTO has a round-square otto
  head (biological version of the same silhouette)
- Where OSCAR has two large round simple eyes (black with a single small
  white highlight), OTTO has the same eye style
- Where OSCAR has a simple curved smile, OTTO has the same smile
- Where OSCAR has three short horizontal "speaker" lines on each cheek,
  OTTO has three short whisker lines on each cheek
- Where OSCAR has small darker "speaker" accessory shapes on either side of
  the head, OTTO has small rounded otto ears in the same position
- Where OSCAR has three small red lights on top of its head, OTTO has
  three small light-blue water-droplet shapes on top of its head (signaling
  the otto's aquatic nature — same placement, same rhythm, cool accent
  instead of warm)
- Where OSCAR has a small triangular dark element below its chin, OTTO
  has a small triangular black otto nose
...
PALETTE — translate OSCAR's cool blue palette into OTTO's warm earth tones,
keeping the same saturation level and the same value relationships:
- OTTO Brown #8B5E3C — primary fur (replaces OSCAR's baby blue body)
...
| OSCAR element | OTTO equivalent |
|---|---|
| TV-screen face | Round-square otto face (biological) |
| Three red lights on top | Three light-blue water droplets |
| Dark-blue "speaker" ears | Small round otto ears |
| "Speaker" lines on cheeks | Whisker lines on cheeks |
| Triangular dark element under chin | Small triangular otto nose |
| Baby-blue body | OTTO-brown fur with cream belly patch |
...
For the activity-flow infographic, OTTO carries a small slate tile with a glowing `>_` terminal prompt (echoing "ottos carry rocks as tools"). The brand-mascot version above deliberately omits that prop so the silhouette stays sibling-clean with OSCAR. Generate the prop variant from the same base character later if needed.

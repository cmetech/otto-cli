...
**Related:** ADR-003 (pipeline simplification), [Issue #2655](https://github.com/cmetech/otto-cli/issues/2655), `docs/dynamic-model-routing.md`

## Context

OTTO already supports dynamic model routing in auto-mode, but the current router is fundamentally **complexity-tier and cost based**, not **task-capability based**.
...
The current router cannot express those differences. If Claude and Codex are both available at the same tier, OTTO either:
...
Experienced users know which models are better at coding, debugging, research, long-context work, or instruction following. OTTO has no representation for that knowledge, so it cannot route intelligently on the user's behalf.
...
Built-in capability profiles are maintained alongside the existing `MODEL_CAPABILITY_TIER` and `MODEL_COST_PER_1K_INPUT` tables in `model-router.ts`. When the `@otto/pi-ai` model catalog is updated with new models, the capability profile table must be updated in the same PR. A linting rule should flag any model present in `MODEL_CAPABILITY_TIER` but missing from `MODEL_CAPABILITY_PROFILES`.

Profiles are versioned implicitly by OTTO release. The existing `models.json` `modelOverrides` mechanism allows users to correct stale defaults immediately without waiting for a OTTO update.
...
Built-in model profiles will drift as model families evolve. Mitigation: profiles live in a single data table, versioned with OTTO releases, with a lint rule for completeness.
...
If the default profiles are not reviewed regularly, OTTO will encode outdated assumptions about which models are "best" at which tasks.
...
Rejected because it breaks as soon as the user does not have the expected model. This is appropriate for a closed product with a fixed fleet, not for OTTO's user-configured provider model.

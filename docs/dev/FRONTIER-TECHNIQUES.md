# Frontier Techniques for OTTO

Research into cutting-edge AI agent techniques that map directly to OTTO's architecture, ranked by impact and feasibility.
...
OTTO is a multi-layered, event-driven agent platform with strong extensibility primitives: a skill system, file-based memory, session branching, compaction, and 16+ extension lifecycle hooks. These existing primitives create natural integration points for six frontier techniques that could fundamentally change how OTTO operates.

The techniques fall into three categories:

| Category | Techniques | Theme |
|----------|-----------|-------|
| **Self-Improvement** | Skill Library Evolution, Cross-Session Learning Graph | OTTO gets better the more you use it |
| **Performance** | DAG Tool Execution, Speculative Tool Execution | OTTO gets faster per turn |
| **Intelligence** | Semantic Context Compression, MCTS Planning | OTTO reasons better with the same context budget |
...
Inspired by [SkillRL](https://arxiv.org/abs/2602.08234) (ICLR 2026), this technique transforms OTTO's skill system from static instruction files into a self-improving knowledge base. Instead of skills being written once and updated manually, they evolve based on execution outcomes.
...
OTTO already has every primitive needed:
...
| OTTO Component | Role in Integration |
|---------------|-------------------|
...
**Current OTTO behavior (sequential):**
```
...
The model already emits multiple `tool_use` blocks in a single response. OTTO processes them, but the execution path in `agent-loop.ts` handles them in sequence. The parallelism opportunity is sitting right there.
...
The #1 latency bottleneck in OTTO is the round-trip: user prompt → model thinks → model requests tool → tool executes → result sent back → model thinks again. Speculative execution attacks the highest-latency step.

OTTO's architecture makes this easy to add:
...
OTTO's memory system (MEMORY.md + individual files) stores flat, file-based memories. A learning graph extends this into a structured knowledge base that captures relationships between codebases, files, errors, solutions, and patterns across all sessions.
...
Inspired by [ToolTree](https://www.agentic-patterns.com/patterns/skill-library-evolution/) and Monte Carlo Tree Search, this technique replaces OTTO's linear action selection with a tree-based planner that explores multiple solution paths simultaneously.
...
OTTO already has session branching primitives:

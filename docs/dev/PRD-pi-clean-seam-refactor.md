...
|-----------|-------------|
| Zero GSD business logic in vendored pi packages | `pi-coding-agent/src/` contains no files that import from `@loop24/` packages (except the extension system's bundled module map) |
...
| Applying a pi-mono update is scoped | Updating pi packages produces type errors only in `@loop24/agent-core` and `@loop24/agent-modes` — no changes required in pi package source files |
...
- **Extension authors** — must not be broken; the extension API surface stays in `@loop24/pi-coding-agent`
- **End users** — not impacted; the refactor is entirely internal

## Requirements

### R1 — New package: `@loop24/agent-core`

A new workspace package at `packages/gsd-agent-core/` that owns all GSD session orchestration logic. It depends on `@loop24/pi-coding-agent`, `@loop24/pi-agent-core`, and `@loop24/pi-ai`. Nothing in the vendored pi packages depends on it.
...
### R2 — New package: `@loop24/agent-modes`

A new workspace package at `packages/gsd-agent-modes/` that owns all run-mode and CLI code. It depends on `@loop24/agent-core`, `@loop24/pi-coding-agent`, and `@loop24/pi-tui`. It is the layer the top-level `gsd-pi` binary entry point assembles.
...
- Import from `@loop24/agent-core` or `@loop24/agent-modes`
- Contain GSD business logic (compaction, session management, run modes, CLI)

The extension system (`src/core/extensions/`) remains in `pi-coding-agent` because it is legitimately pi-typed: extension authors write against pi's `AgentMessage`, `Model`, and `TUI` types. The virtual module map in `extensions/loader.ts` must be updated to include `@loop24/agent-core` and `@loop24/agent-modes` so extensions can import from them.
...
- `web/bridge-service.ts` currently imports `AgentSessionEvent` from an internal path in `pi-coding-agent` — this must be fixed to use the public export from `@loop24/agent-core`
...
1. `@loop24/pi-agent-core`, `@loop24/pi-ai`, `@loop24/pi-tui` (parallel, no dependencies between them)
2. `@loop24/pi-coding-agent`
3. `@loop24/agent-core`
4. `@loop24/agent-modes`
5. `gsd-pi` (top-level binary)

### R6 — No change to the extension loader's public interface

Extensions are loaded by `pi-coding-agent`'s jiti-based loader. The virtual module map (`STATIC_BUNDLED_MODULES`) must be updated to resolve `@loop24/agent-core` and `@loop24/agent-modes` alongside the existing pi package mappings. This requires both a map entry and a top-level bundle import in `loader.ts` (see ADR-009 for the exact diff). Extension authors must not need to change their import paths.
...
2. Does `buildSessionContext()` on `SessionManager` need a public re-export from `@loop24/agent-core`?
3. Should `@loop24/agent-modes` re-export `createAgentSession()` as a convenience, or should consumers always import it from `@loop24/agent-core` directly?

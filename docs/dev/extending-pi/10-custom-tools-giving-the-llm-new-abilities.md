...
For string enum parameters, you **must** use `StringEnum` from `@otto/pi-ai`. `Type.Union([Type.Literal("a"), Type.Literal("b")])` does NOT work with Google's API.
...
```typescript
import { createReadTool, createBashTool } from "@otto/pi-coding-agent";

...
> These changes would benefit every consumer of the pi engine, not just OTTO.
...
**Expected savings**: 80–90% reduction in input token cost for multi-turn sessions (the dominant cost pattern in OTTO auto-mode).
...
- Searches from `~/.otto/agent/` → ancestor dirs → cwd
...
**Current state**: `SessionManager.getUsageTotals()` accumulates cost across the entire session. No per-phase or per-agent breakdown is stored. Cost visibility is limited to the footer total and `OTTO_SHOW_TOKEN_COST=1` per-turn display.
...
OTTO extension could consume these events to surface per-milestone cost in `/otto stats` and flag milestones that are disproportionately expensive — enabling budget-aware planning.

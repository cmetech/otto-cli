...
| `tests/smoke/test-version.ts` | Verify `otto --version` outputs valid semver |
| `tests/smoke/test-help.ts` | Verify `otto --help` exits 0 and contains expected output |
| `tests/smoke/test-init.ts` | Verify `otto init` creates expected files in a temp dir |
...
| `tests/fixtures/record.ts` | Recording helper — runs a session with `OTTO_FIXTURE_MODE=record` |
...
# ──────────────────────────────────────────────
# Stage 1: CI Builder
# Image: ghcr.io/cmetech/otto-ci-builder
...
Run: `docker build --target builder -t otto-ci-builder-test .`
Expected: Completes successfully (may take 5-10 min first time)

- [ ] **Step 3: Verify runtime stage builds**

Run: `docker build --target runtime -t otto-pi-test .`
Expected: Completes successfully

- [ ] **Step 4: Verify runtime image works**

Run: `docker run --rm otto-pi-test --version`
...
// Verifies that `otto --version` outputs valid semver-like string.
// When OTTO_SMOKE_BINARY is set (CI), uses that binary directly.
// Otherwise falls back to npx @cmetech/otto.
...
const output = bin
  ...
  : execFileSync("npx", ["@cmetech/otto", "--version"], { encoding: "utf8", timeout: 30_000 }).trim();
...
// Verifies that `otto --help` exits 0 and contains expected keywords.
...
const output = bin
  ...
  : execFileSync("npx", ["@cmetech/otto", "--help"], { encoding: "utf8", timeout: 30_000 });

const requiredKeywords = ["otto", "usage"];
...
// Verifies that `otto init` creates expected files in a temp directory.
...
const tmp = mkdtempSync(join(tmpdir(), "otto-smoke-init-"));

try {
  const bin = process.env.OTTO_SMOKE_BINARY;
  const args = bin ? [bin, "init"] : ["npx", "@cmetech/otto", "init"];
  execFileSync(args[0], args.slice(1), {
    encoding: "utf8",
    cwd: tmp,
    timeout: 30_000,
    env: { ...process.env, OTTO_NON_INTERACTIVE: "1" },
  });

  // Check that .otto/workflow directory was created
  if (!existsSync(join(tmp, ".otto/workflow"))) {
    console.error("Expected .otto/workflow/ directory not found after init");
    process.exit(1);
  }
...
//   OTTO_FIXTURE_MODE=record|replay
//   OTTO_FIXTURE_DIR=./tests/fixtures/recordings
...
export function getFixtureMode(): FixtureMode {
  const mode = process.env.OTTO_FIXTURE_MODE;
  if (mode === "record" || mode === "replay") return mode;
  return "off";
}

export function getFixtureDir(): string {
  return process.env.OTTO_FIXTURE_DIR || join(process.cwd(), "tests/fixtures/recordings");
}
...
Note: This provider implements the core recording/replay data structures and utilities. Wiring it into the `pi-ai` registry as a drop-in `ApiProvider` (via `registerApiProvider()` from `packages/pi-ai/src/api-registry.ts`) requires importing `@otto/pi-ai` internals, which couples tests to the build output. This integration is deferred to a follow-up task after the pipeline is operational. The current implementation validates fixture format, turn sequencing, and replay correctness independently.
...
```typescript
// tests/live/run.ts
// Runs real LLM integration tests. Only executes when OTTO_LIVE_TESTS=1.
// These tests cost real money — used in the Prod gate only.
//
// Usage: OTTO_LIVE_TESTS=1 node --experimental-strip-types tests/live/run.ts

if (process.env.OTTO_LIVE_TESTS !== "1") {
  console.log("Skipping live tests (set OTTO_LIVE_TESTS=1 to enable)");
  process.exit(0);
}
...
Expected: `Skipping live tests (set OTTO_LIVE_TESTS=1 to enable)` and exit 0
...
jobs:
  ...
  dev-publish:
    ...
    container:
      image: ghcr.io/cmetech/otto-ci-builder:latest  # Pin to date tag after first build
    environment: dev
    outputs:
  ...
  test-verify:
    ...
    steps:
      ...
      - name: Install published dev package globally
        run: npm install -g @cmetech/otto@dev
      ...
      - name: Run CLI smoke tests
        run: npm run test:smoke
        env:
          OTTO_SMOKE_BINARY: otto  # Use globally installed binary, not npx

      - name: Run fixture replay tests
        run: npm run test:fixtures

      - name: Promote to @next
        run: npm dist-tag add @cmetech/otto@${{ needs.dev-publish.outputs.dev-version }} next
        env:
          NODE_AUTH_TOKEN: ${{ secrets.NPM_TOKEN }}
      ...
      - name: Build and push runtime Docker image
        run: |
          ...
          docker build --target runtime \
            --build-arg OTTO_VERSION=${{ needs.dev-publish.outputs.dev-version }} \
            -t ghcr.io/cmetech/otto:next \
            -t ghcr.io/cmetech/otto:${{ needs.dev-publish.outputs.dev-version }} \
            .
          docker push ghcr.io/cmetech/otto:next
          docker push ghcr.io/cmetech/otto:${{ needs.dev-publish.outputs.dev-version }}
    ...
    steps:
      ...
      - name: Run live LLM tests (optional)
        ...
        run: |
          npm ci
          npm run build
          OTTO_LIVE_TESTS=1 npm run test:live
        env:
          ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
          OPENAI_API_KEY: ${{ secrets.OPENAI_API_KEY }}

      - name: Promote to @latest
        run: npm dist-tag add @cmetech/otto@${{ needs.dev-publish.outputs.dev-version }} latest
        env:
          NODE_AUTH_TOKEN: ${{ secrets.NPM_TOKEN }}
      ...
      - name: Tag and push Docker images
        run: |
          echo "${{ secrets.GITHUB_TOKEN }}" | docker login ghcr.io -u ${{ github.actor }} --password-stdin
          docker pull ghcr.io/cmetech/otto:${{ needs.dev-publish.outputs.dev-version }}
          docker tag ghcr.io/cmetech/otto:${{ needs.dev-publish.outputs.dev-version }} ghcr.io/cmetech/otto:latest
      ...
      - name: Post-publish smoke test
        run: |
          ...
          npm init -y
          npm install @cmetech/otto@latest
          npx otto --version
      ...
      - name: Build and push CI builder image
        run: |
          ...
          docker build --target builder \
            -t ghcr.io/cmetech/otto-ci-builder:latest \
            -t ghcr.io/cmetech/otto-ci-builder:${{ steps.tag.outputs.date }} \
            .
          docker push ghcr.io/cmetech/otto-ci-builder:latest
          docker push ghcr.io/cmetech/otto-ci-builder:${{ steps.tag.outputs.date }}

      - name: Verify builder image
        run: |
          docker run --rm ghcr.io/cmetech/otto-ci-builder:latest node --version
          docker run --rm ghcr.io/cmetech/otto-ci-builder:latest rustc --version
```
  ...
  cleanup:
    ...
    steps:
      ...
      - name: Remove old dev versions
        run: |
          VERSIONS=$(npm view @cmetech/otto versions --json 2>/dev/null || echo "[]")
          ...
          for VERSION in $DEV_VERSIONS; do
            PUBLISH_TIME=$(npm view "@cmetech/otto@$VERSION" time --json 2>/dev/null || echo "")
            ...
            if [ -n "$PUBLISH_TIME" ]; then
              ...
              if [ "$AGE_MS" -gt "$THIRTY_DAYS_MS" ]; then
                echo "Unpublishing @cmetech/otto@$VERSION"
                npm unpublish "@cmetech/otto@$VERSION" || echo "Failed to unpublish $VERSION"
              else
                echo "Keeping @cmetech/otto@$VERSION (within 30 days)"
              fi
            fi
          done
        env:
...
if (mode !== "record") {
  console.error("Recording requires OTTO_FIXTURE_MODE=record");
  console.error("");
  console.error("Usage:");
  console.error("  OTTO_FIXTURE_MODE=record OTTO_FIXTURE_DIR=./tests/fixtures/recordings \\");
...
console.log("1. Set OTTO_FIXTURE_MODE=record in your environment");
console.log("2. Run your OTTO session normally");
...
- Live tests: Skipped (no `OTTO_LIVE_TESTS=1`)
...
4. **Enable GHCR:**
   - Ensure GitHub Container Registry is enabled for the `cmetech` org

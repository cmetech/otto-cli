...
- **No, you do not have to rely on `~/.otto/workflow`** if you embed pi with custom paths and storage
...
```bash
npm install -g my-otto
# or a standalone binary / packaged desktop app

otto
```

And inside `otto`, you import pi packages and create your own session, UI, storage, and resource loading behavior.

---

## 19.3 The Second Biggest Misconception: `~/.otto/workflow` Is a Default, Not a Requirement

Pi CLI defaults to `~/.otto/agent`, but embedded applications are not forced to use it.
...
This is the most natural fit for tools like `otto`.
...
- easiest way to remove dependency on `~/.otto/workflow`
...
|------|---------------------|
| Branded CLI like `otto` | `@mariozechner/pi-coding-agent` SDK |
...
## 19.6 The Recommended Path for a Branded CLI Like `otto`
...
If your product uses `~/.otto/workflow`, then:
...
const appRoot = path.join(os.homedir(), ".otto/workflow");
...
This is another place where people often assume they must rely on discovery from `~/.otto/workflow` or `.otto/workflow/`.
  ...
  extensionFactories: [
    (pi) => {
      pi.registerCommand("hello", {
        ...
        handler: async (_args, ctx) => ctx.ui.notify("Hello from OTTO", "info"),
      });
    },
  ],
});
...
const loader = new DefaultResourceLoader({
  ...
  systemPromptOverride: () => "You are OTTO, a specialized software delivery agent.",
});
```
...
### Recommendation
For a branded tool like `otto`, default to **bundled-app product** behavior.
...
```typescript
const authStorage = AuthStorage.create("/path/to/otto/auth.json");
```
...
```typescript
const modelRegistry = new ModelRegistry(authStorage, "/path/to/otto/models.json");
```
...
## 19.18 Building a Branded `otto` CLI: Recommended Shape
...
const resourceLoader = new DefaultResourceLoader({
  ...
  systemPromptOverride: () =>
    "You are OTTO, a branded software delivery agent. Prefer project-specific workflows and terminology.",
  additionalExtensionPaths: [
...
If your app silently loads from a user's `~/.otto/workflow`, you may get:
...
If your app is called `otto`, but state lives in `~/.otto/workflow`, users will notice.
...
- Good for productized tools like `otto`
...
> “I want users to download and run `otto`, and have it use pi internally without requiring a separate pi install or `~/.otto/workflow` setup.”

...
// OTTO Extension SDK — Complete API surface reference
...
|------|--------|-------------|
| `ExtensionAPI` | `@otto/pi-coding-agent` | The `pi` object |
| `ExtensionContext` | `@otto/pi-coding-agent` | Context for event handlers |
| `ExtensionCommandContext` | `@otto/pi-coding-agent` | Extended context for command handlers |
| `ExtensionUIContext` | `@otto/pi-coding-agent` | UI methods on `ctx.ui` |
| `ToolDefinition` | `@otto/pi-coding-agent` | Tool registration shape |
| `AgentToolResult` | `@otto/pi-coding-agent` | Tool execution result |
| `ProviderConfig` | `@otto/pi-coding-agent` | Provider registration config |
| `ExtensionFactory` | `@otto/pi-coding-agent` | `(pi: ExtensionAPI) => void \| Promise<void>` |
| `ContextUsage` | `@otto/pi-coding-agent` | `{ tokens, contextWindow, percent }` |
| `AgentAbortOrigin` | `@otto/pi-agent-core` | `"session-transition" \| "user" \| "timeout" \| "unknown"` |
| `ThinkingLevel` | `@otto/pi-agent-core` | `"off" \| "low" \| "medium" \| "high" \| "xhigh"` |

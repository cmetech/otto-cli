...
// OTTO Extension SDK — Non-negotiable rules, gotchas, and contribution requirements

Non-negotiable rules, common gotchas, and contribution requirements for OTTO extensions. Violating any non-negotiable rule will block your PR.
...
Large tool output causes context overflow. Enforce a maximum of **50 KB / 2000 lines**. Use the truncation helpers from `@otto/pi-coding-agent`:

```typescript
import { truncateHead, truncateTail } from "@otto/pi-coding-agent";
...
Use `truncateToWidth()` from `@otto/pi-tui` to enforce line width:
...
| **bundled** | Ships with OTTO, user can disable. Default for new features. |
| **community** | User-installed. Lives in `~/.otto/agent/extensions/`. |

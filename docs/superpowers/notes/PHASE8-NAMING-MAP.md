...
(env vars) where they get LOOP24_X canonical + OTTO_X fallback treatment.
The 23 names below are confirmed module-level constants (0
`process.env.X` references).

| Old | New | Files | Notes |
|---|---|---|---|
| `MISSING_OTTO_MARKER` | `MISSING_WORKFLOW_MARKER` | 4 | `MISSING_WORKFLOW_MARKER` free (0 hits). |
| `OTTO_COMMAND_DESCRIPTION` | `WORKFLOW_COMMAND_DESCRIPTION` | 4 | `WORKFLOW_COMMAND_DESCRIPTION` free (0 hits). |
| `OTTO_GIT_ERROR` | `GIT_ERROR` | 9 | `GIT_ERROR` free (0 hits). |
| `OTTO_IO_ERROR` | `IO_ERROR` | 4 | `IO_ERROR` free (0 hits). |
| `OTTO_PARSE_ERROR` | `PARSE_ERROR` | 5 | `PARSE_ERROR` free (0 hits). |
| `OTTO_MERGE_CONFLICT` | `MERGE_CONFLICT` | 3 | `MERGE_CONFLICT` free (0 hits). |
| `OTTO_BLOCKED` | `WORKFLOW_BLOCKED` | 1 | `BLOCKED` has 3 hits in GitHub workflow scripts (string literals, not code constants); `WORKFLOW_BLOCKED` is code-identifier-free (0 hits). |
| `OTTO_ARTIFACT_MISSING` | `ARTIFACT_MISSING` | 1 | `ARTIFACT_MISSING` free (0 hits). |
| `OTTO_LOCK_HELD` | `LOCK_HELD` | 2 | `LOCK_HELD` free (0 hits). |
| `OTTO_ALLOWED` | `ALLOWED` | 1 | `ALLOWED` free (0 hits). |
| `OTTO_STALE_STATE` | `STALE_STATE` | 6 | `STALE_STATE` free (0 hits). |
| `OTTO_LOGO` | `LOGO` | 2 | `LOGO` free (0 hits). |
| `OTTO_ROOT_FILES` | `ROOT_FILES` | 2 | `ROOT_FILES` free (0 hits). |
| `OTTO_ROOT_TTL_MS` | `ROOT_TTL_MS` | 1 | `ROOT_TTL_MS` free (0 hits). |
| `OTTO_SHORTCUTS` | `SHORTCUTS` | 2 | `SHORTCUTS` free (0 hits). |
| `OTTO_RUNTIME_PATTERNS` | `RUNTIME_PATTERNS` | 4 | `RUNTIME_PATTERNS` free (0 hits). |
| `OTTO_NUMBERED_VARIANT_RE` | `NUMBERED_VARIANT_RE` | 1 | `NUMBERED_VARIANT_RE` free (0 hits). |
| `OTTO_DIR_RE` | `DIR_RE` | 1 | `DIR_RE` free (0 hits). |
| `OTTO_SNAPSHOT_PREFIX` | `SNAPSHOT_PREFIX` | 1 | `SNAPSHOT_PREFIX` free (0 hits). |
| `OTTO_STATUS_KEYS` | `STATUS_KEYS` | 1 | `STATUS_KEYS` free (0 hits). |
| `OTTO_WIDGET_KEYS` | `WIDGET_KEYS` | 1 | `WIDGET_KEYS` free (0 hits). |
| `OTTO_DAEMON_CONFIG` | `DAEMON_CONFIG` | 2 | `DAEMON_CONFIG` free (0 hits). |
| `OTTO_PHASE_INACTIVE` | `PHASE_INACTIVE` | 1 | `PHASE_INACTIVE` free (0 hits). |

**Reclassified to Section 5 (env vars):** `OTTO_MILESTONE_LOCK`, `OTTO_PARALLEL_WORKER`, `OTTO_WORKFLOW_EXECUTORS_MODULE`, `OTTO_WORKFLOW_WRITE_GATE_MODULE`, `OTTO_PERSIST_WRITE_GATE_STATE`, `OTTO_SLICE_LOCK`, `OTTO_ENGINE_BYPASS`, `OTTO_ALLOWED_COMMAND_PREFIXES`, `OTTO_BUNDLED_EXTENSION_PATHS`, `OTTO_WORKTREE`, `OTTO_CLI_WORKTREE`, `OTTO_CLI_WORKTREE_BASE`, `OTTO_STARTUP_TIMING`, `OTTO_SHOW_TOKEN_COST`, `OTTO_VERBOSE`.

## 5. Env vars (LOOP24_X canonical + OTTO_X fallback)

The 11 vars explicitly listed in the Phase 8 plan, **plus 4 additional env vars confirmed during 2026-05-24 open-questions resolution** (originally suspected to be module constants; confirmed as `process.env` reads/writes). For all others in the broader `OTTO_*` constant list (Section 4): those are internal module constants.

| Old | New (canonical) | All sites | Setter sites | Notes |
|---|---|---|---|---|
| `OTTO_DEBUG` | `LOOP24_DEBUG` | 9 | 2 | `LOOP24_DEBUG` free (0 existing hits). |
| `OTTO_HOME` | `LOOP24_HOME` | 65 | 43 | **Already partially deployed.** `LOOP24_HOME` has 3 existing hits in `src/app-paths.ts` and `src/loop24-config.ts` (Phase 0 canonical). Task: add `OTTO_HOME` fallback shim alongside existing `LOOP24_HOME` reads; deprecate setter. |
| `OTTO_PKG_ROOT` | `LOOP24_PKG_ROOT` | 7 | 2 | `LOOP24_PKG_ROOT` free (0 existing hits). |
| `OTTO_WORKFLOW_PATH` | `LOOP24_WORKFLOW_PATH` | 10 | 6 | `LOOP24_WORKFLOW_PATH` free (0 existing hits). |
| `OTTO_CODING_AGENT_DIR` | `LOOP24_CODING_AGENT_DIR` | 10 | 3 | `LOOP24_CODING_AGENT_DIR` free (0 existing hits). |
| `OTTO_VERSION` | `LOOP24_VERSION` | 15 | 4 | `LOOP24_VERSION` free (0 existing hits). |
| `OTTO_FIRST_RUN_BANNER` | `LOOP24_FIRST_RUN_BANNER` | 2 | 2 | **Already partially deployed.** `LOOP24_FIRST_RUN_BANNER` has 1 existing hit in `src/loader.ts` with a TODO comment noting the legacy alias still in place. Task: remove the `OTTO_FIRST_RUN_BANNER` side. |
| `OTTO_BIN_PATH` | `LOOP24_BIN_PATH` | 14 | 2 | `LOOP24_BIN_PATH` free (0 existing hits). |
| `OTTO_SKIP_RTK_INSTALL` | `LOOP24_SKIP_RTK_INSTALL` | 3 | 1 | `LOOP24_SKIP_RTK_INSTALL` free (0 existing hits). Note: `OTTO_SKIP_RTK_INSTALL_ENV` (2 hits) is likely a string constant holding the env var name — must update its value too. |
| `OTTO_RTK_DISABLED` | `LOOP24_RTK_DISABLED` | 10 | 4 | `LOOP24_RTK_DISABLED` free (0 existing hits). Note: `OTTO_RTK_DISABLED_ENV` (2 hits) is a string constant holding the env var name — must update its value. |
| `OTTO_TEST_CLONE_MARKETPLACES` | `LOOP24_TEST_CLONE_MARKETPLACES` | 1 | 1 | `LOOP24_TEST_CLONE_MARKETPLACES` free (0 existing hits). |
| `OTTO_PROJECT_ROOT` | `LOOP24_PROJECT_ROOT` | 47 | 6+ | **Added 2026-05-24.** Confirmed env var: `process.env.OTTO_PROJECT_ROOT = projectRoot` (auto.ts:356); reader `process.env.OTTO_PROJECT_ROOT?.trim()` in worktree-root.ts; spread into worker process envs in parallel-orchestrator.ts and slice-parallel-orchestrator.ts. `LOOP24_PROJECT_ROOT` free (0 existing hits). |
| `OTTO_WORKFLOW_PROJECT_ROOT` | `LOOP24_WORKFLOW_PROJECT_ROOT` | 47 | 1 | **Added 2026-05-24.** Confirmed env var: set in workflow-mcp.ts:216 (`OTTO_WORKFLOW_PROJECT_ROOT: projectRoot`) and read at :235-236. Used in MCP server env spreads. `LOOP24_WORKFLOW_PROJECT_ROOT` free (0 existing hits). |
| `OTTO_ENABLE_NATIVE_OTTO_GIT` | `LOOP24_ENABLE_NATIVE_GIT` | 3 | 0 | **Added 2026-05-24.** Confirmed env var: `process.env.OTTO_ENABLE_NATIVE_OTTO_GIT === "1"` in native-git-bridge.ts:19. Feature flag for native git path. Inner duplicate `GSD` dropped; "native" already qualifies the feature. `LOOP24_ENABLE_NATIVE_GIT` free (0 existing hits). |
| `OTTO_ENABLE_NATIVE_OTTO_PARSER` | `LOOP24_ENABLE_NATIVE_PARSER` | 1 | 0 | **Added 2026-05-24.** Confirmed env var: `process.env.OTTO_ENABLE_NATIVE_OTTO_PARSER === "1"` in native-parser-bridge.ts:11. Feature flag for native parser path. `LOOP24_ENABLE_NATIVE_PARSER` free (0 existing hits). |
| `OTTO_MILESTONE_LOCK` | `LOOP24_MILESTONE_LOCK` | 34 | many | **Reclassified 2026-05-24** from Section 4 during Task 7 audit. Used directly via `process.env.OTTO_MILESTONE_LOCK`. Parallel-worker coordination env. `LOOP24_MILESTONE_LOCK` free. |
| `OTTO_PARALLEL_WORKER` | `LOOP24_PARALLEL_WORKER` | 40 | many | **Reclassified 2026-05-24.** Parallel-worker mode flag. `LOOP24_PARALLEL_WORKER` free. |
| `OTTO_WORKFLOW_EXECUTORS_MODULE` | `LOOP24_WORKFLOW_EXECUTORS_MODULE` | 10 | many | **Reclassified 2026-05-24.** Module-path override env. `LOOP24_WORKFLOW_EXECUTORS_MODULE` free. |
| `OTTO_WORKFLOW_WRITE_GATE_MODULE` | `LOOP24_WORKFLOW_WRITE_GATE_MODULE` | 4 | many | **Reclassified 2026-05-24.** Module-path override env. `LOOP24_WORKFLOW_WRITE_GATE_MODULE` free. |
| `OTTO_PERSIST_WRITE_GATE_STATE` | `LOOP24_PERSIST_WRITE_GATE_STATE` | 20 | many | **Reclassified 2026-05-24.** Write-gate persistence flag. `LOOP24_PERSIST_WRITE_GATE_STATE` free. |
| `OTTO_SLICE_LOCK` | `LOOP24_SLICE_LOCK` | 11 | many | **Reclassified 2026-05-24.** Parallel-worker slice-lock env. `LOOP24_SLICE_LOCK` free. |
| `OTTO_ENGINE_BYPASS` | `LOOP24_ENGINE_BYPASS` | 6 | many | **Reclassified 2026-05-24.** Engine bypass flag. `LOOP24_ENGINE_BYPASS` free. |
| `OTTO_ALLOWED_COMMAND_PREFIXES` | `LOOP24_ALLOWED_COMMAND_PREFIXES` | 6 | many | **Reclassified 2026-05-24.** Allowed-prefixes override env. `LOOP24_ALLOWED_COMMAND_PREFIXES` free. |
| `OTTO_BUNDLED_EXTENSION_PATHS` | `LOOP24_BUNDLED_EXTENSION_PATHS` | 5 | many | **Reclassified 2026-05-24.** Bundled-extension-paths override env. `LOOP24_BUNDLED_EXTENSION_PATHS` free. |
| `OTTO_WORKTREE` | `LOOP24_WORKTREE` | 5 | many | **Reclassified 2026-05-24.** Worktree-mode env. `LOOP24_WORKTREE` free. |
| `OTTO_CLI_WORKTREE` | `LOOP24_CLI_WORKTREE` | 5 | many | **Reclassified 2026-05-24.** CLI worktree path env. `LOOP24_CLI_WORKTREE` free. |
| `OTTO_CLI_WORKTREE_BASE` | `LOOP24_CLI_WORKTREE_BASE` | 5 | many | **Reclassified 2026-05-24.** CLI worktree base path env. `LOOP24_CLI_WORKTREE_BASE` free. |
| `OTTO_STARTUP_TIMING` | `LOOP24_STARTUP_TIMING` | 2 | 0 | **Reclassified 2026-05-24.** Startup timing diagnostic flag (also has `PI_TIMING` alias). `LOOP24_STARTUP_TIMING` free. |
| `OTTO_SHOW_TOKEN_COST` | `LOOP24_SHOW_TOKEN_COST` | 2 | many | **Reclassified 2026-05-24.** Token cost UI toggle. `LOOP24_SHOW_TOKEN_COST` free. |
| `OTTO_VERBOSE` | `LOOP24_VERBOSE` | 1 | 0 | **Reclassified 2026-05-24.** Verbose logging flag. `LOOP24_VERBOSE` free. |

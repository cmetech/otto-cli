...
For OTTO2, the safe path is:

1. Treat local Claude Code as an external authenticated runtime.
2. Never ask OTTO users to sign into Claude subscriptions through OTTO-managed Anthropic OAuth.
3. Never exchange Claude.ai subscription OAuth into a bearer token and call Anthropic APIs as if OTTO were Claude Code.
4. If OTTO needs direct Anthropic API access, require a Claude Console API key, Bedrock, Vertex, or another explicitly supported provider path.
...
- If OTTO2 shells out to or embeds Claude Code, it should respect Claude Code's own credential selection instead of inventing a parallel Anthropic OAuth flow.
...
- If OTTO2 is acting as a product for users, direct Anthropic access should be through commercial auth paths, not subscription-token reuse.
...
- A OTTO-managed Anthropic OAuth flow for subscription accounts is high risk.
- Reusing user Claude subscription credentials inside OTTO's own API client is high risk.
- Any flow that makes Anthropic believe requests come from Claude Code when they actually come from OTTO infrastructure is out of bounds.

## Current OTTO2 Findings
...
These are directionally correct because OTTO is using the user's own local Claude Code installation as the authenticated Anthropic surface.
...
- OTTO2 must not implement its own Anthropic subscription OAuth flow for end users.
- OTTO2 must not persist Anthropic subscription OAuth tokens for later API use.
- OTTO2 must not send Anthropic API traffic using subscription OAuth tokens obtained by OTTO.
- OTTO2 may support Anthropic direct access only via:
  - `ANTHROPIC_API_KEY`
...
4. Add explicit UI copy:
   - "Claude subscription users: sign into the local Claude Code app/CLI, not OTTO."
5. Block migrations or code paths that convert Anthropic OAuth credentials into API auth for OTTO-managed requests.
...
If a proposed OTTO2 feature needs Anthropic access, ask one question:

"Is OTTO calling Anthropic as OTTO, or is OTTO delegating to the user's already-authenticated local Claude Code runtime?"

- If OTTO is calling Anthropic as OTTO: require API key or supported cloud auth.
- If OTTO is delegating to local Claude Code: acceptable, as long as OTTO does not intercept, mint, or replay subscription credentials itself.

...
OTTO tracks token usage and cost for every unit of work dispatched during auto mode. This data powers the dashboard, budget enforcement, and cost projections.
...
Data is stored in `.otto/workflow/metrics.json` and survives across sessions.

### Viewing Costs

**Dashboard:** `Ctrl+Alt+G` or `/otto status` shows real-time cost breakdown.
...
Once at least two slices have completed, OTTO projects the remaining cost:
...
- Check `/otto status` after a few slices to see per-slice cost averages
...
- Use `/otto visualize` → Metrics tab to see where your budget is going

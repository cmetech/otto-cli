...
Add custom providers and models (Ollama, vLLM, LM Studio, proxies) via `~/.otto/agent/models.json`.
...
Some OpenAI-compatible servers do not understand the `developer` role used for reasoning-capable models. For those providers, set `compat.supportsDeveloperRole` to `false` so OTTO sends the system prompt as a `system` message instead. If the server also does not support `reasoning_effort`, set `compat.supportsReasoningEffort` to `false` too.
...
If you use a credential tool not on the default list, override it in global settings (`~/.otto/agent/settings.json`):
...
Alternatively, set the `OTTO_ALLOWED_COMMAND_PREFIXES` environment variable (comma-separated). The env var takes precedence over settings.json:

```bash
export OTTO_ALLOWED_COMMAND_PREFIXES="pass,op,sops,doppler"
```

> **Note:** This setting is global-only. Project-level settings.json (`<project>/.otto/workflow/settings.json`) cannot override the command allowlist — this prevents a cloned repo from escalating command execution privileges.
...
| `reasoningEffortMap` | Map OTTO thinking levels to provider-specific `reasoning_effort` values |

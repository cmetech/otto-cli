...
| **状态推导** | 通过 `OTTO_MILESTONE_LOCK` 环境变量，让 `deriveState()` 只看到被分配的 milestone |
...
| **`OTTO_MILESTONE_LOCK`** | 每个 worker 在状态推导时只能看到自己的 milestone |
| **`OTTO_PARALLEL_WORKER`** | Worker 不能再嵌套启动新的并行会话 |

...
const USAGE = `Usage: otto-daemon [options]

Options:
  --config <path>  Path to YAML config file (default: ~/.otto/daemon.yaml)
...
async function main(): Promise<void> {
  ...
  if (values.install) {
    ...
    process.stdout.write('otto-daemon: launchd agent installed and loaded.\n');
    process.exit(0);
  }

  if (values.uninstall) {
    uninstall();
    process.stdout.write('otto-daemon: launchd agent uninstalled.\n');
    process.exit(0);
  }
  ...
  if (values.status) {
    ...
    if (!result.registered) {
      process.stdout.write('otto-daemon: not registered with launchd.\n');
    } else if (result.pid != null) {
      process.stdout.write(
        `otto-daemon: running (PID ${result.pid}, last exit status: ${result.lastExitStatus ?? 'n/a'})\n`,
      );
    } else {
      process.stdout.write(
        `otto-daemon: registered but not running (last exit status: ${result.lastExitStatus ?? 'n/a'})\n`,
      );
    }
    process.exit(0);
  }
...
main().catch((err: unknown) => {
  ...
  process.stderr.write(`otto-daemon: fatal: ${msg}\n`);
  process.exit(1);
});

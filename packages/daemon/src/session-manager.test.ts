 ...
 * and cleanup without spawning real OTTO processes.
 */
...
describe('SessionManager', () => {
  ...
  it('sends custom command when provided', async () => {
    ...
    await manager.startSession({ projectDir: '/tmp/custom-cmd', command: '/otto quick fix-typo' });
    const client = manager.lastClient!;

    assert.ok(client.prompted.includes('/otto quick fix-typo'));
    assert.ok(!client.prompted.includes('/otto auto'));
  });

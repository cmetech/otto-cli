...
function getWriteGateModuleCandidates(): string[] {
  ...
  candidates.push(
    ...buildBridgeImportCandidates("../../../src/resources/extensions/workflow/bootstrap/write-gate.js")
...
function warnCustomWorkflowModule(varName: string, value: string): void {
  ...
  process.stderr.write(
    `[otto-mcp-server] WARNING: ${varName} is set (${value}). ` +
...
async function loadProjectPreferences(projectDir: string): Promise<unknown | null> {
  const { loadEffectiveGSDPreferences } = await importLocalModule<any>(
    "../../../src/resources/extensions/workflow/preferences.js",
  );
  try {
...
function getWorkflowExecutorModuleCandidates(env: NodeJS.ProcessEnv = process.env): string[] {
  ...
  candidates.push(
    ...buildBridgeImportCandidates("../../../src/resources/extensions/workflow/tools/workflow-tool-executors.js")
      .map((p) => new URL(p, import.meta.url).href),
  );
...
async function runSerializedWorkflowDbOperation<T>(
  ...
  return runSerializedWorkflowOperation(async () => {
    const { ensureDbOpen } = await importLocalModule<WorkflowDbBootstrapModule>(
      "../../../src/resources/extensions/workflow/bootstrap/dynamic-tools.js",
    );
...
async function generateOrReuseMilestoneId(projectDir: string): Promise<string> {
  const {
    ...
    nextMilestoneId,
    milestoneIdSort,
  } = await importLocalModule<any>("../../../src/resources/extensions/workflow/milestone-ids.js");
  ...
  const { isReusableGhostMilestone } = await importLocalModule<any>(
    "../../../src/resources/extensions/workflow/state.js",
  );
...
export function registerWorkflowTools(realServer: McpToolServer): void {
  ...
  server.tool(
    ...
    async (args: Record<string, unknown>) => {
      ...
      await runSerializedWorkflowDbOperation(projectDir, async () => {
        ...
        const { rebuildState } = await importLocalModule<any>("../../../src/resources/extensions/workflow/doctor.js");
  ...
  server.tool(
    ...
    async (args: Record<string, unknown>) => {
      ...
      const { queryJournal } = await importLocalModule<any>("../../../src/resources/extensions/workflow/journal.js");
  ...
  server.tool(
    ...
    async (args: Record<string, unknown>) => {
      ...
      const { executeExecSearch } = await importLocalModule<any>(
        "../../../src/resources/extensions/workflow/tools/exec-search-tool.js",
      );
      return adaptExecutorResult(
  ...
  server.tool(
    ...
    async (args: Record<string, unknown>) => {
      ...
      const { executeResume } = await importLocalModule<any>(
        "../../../src/resources/extensions/workflow/tools/resume-tool.js",
      );
      return adaptExecutorResult(

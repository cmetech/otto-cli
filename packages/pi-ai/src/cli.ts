...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @otto/pi-ai <command> [provider]
...
Examples:
  npx @otto/pi-ai login              # interactive provider selection
  npx @otto/pi-ai login anthropic    # login to specific provider
  npx @otto/pi-ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @otto/pi-ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @otto/pi-ai --help' for usage`);
	process.exit(1);
}

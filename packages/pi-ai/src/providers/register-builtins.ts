...
/**
 * E2E-test-only: when `OTTO_FAKE_LLM_TRANSCRIPT` is set, register a
...
function registerFakeProviderIfEnabled(): void {
	const transcriptPath = (process.env.OTTO_FAKE_LLM_TRANSCRIPT ?? process.env.OTTO_FAKE_LLM_TRANSCRIPT);

...
import type { AgentTool, AgentToolResult, AgentToolUpdateCallback } from "@otto/pi-agent-core";
import {
	ensureFileOpen,
	getActiveClients,
	getOrCreateClient,
			...
			if (action === "status") {
				...
				if (servers.length > 0) {
					...
					if (matchedButMissing.length > 0) {
						...
						diagnostics.push("\nInstall the missing server command and restart OTTO, or run: lsp reload");
					} else {

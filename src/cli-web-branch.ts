...
function emitWebModeFailure(stderr: WritableLike, status: WebModeLaunchStatus): void {
  if (status.ok) return
  stderr.write(`[otto] Web mode launch failed: ${status.failureReason}\n`)
}
...
export async function runWebCliBranch(
  ...
  if (webPath) {
    ...
    if (!checkExists(currentCwd)) {
      stderr.write(`[otto] Project path does not exist: ${currentCwd}\n`)
      return {
        handled: true,
        exitCode: 1,
        action: 'start',
    ...
    stderr.write(`[otto] Using project path: ${currentCwd}\n`)
  } else {

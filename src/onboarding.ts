...
import type { AuthStorage } from '@otto/pi-coding-agent'
...
async function loadClack(): Promise<ClackModule> {
  try {
    ...
    throw new Error(`[${COMMAND_NAMESPACE}] @clack/prompts not found — onboarding wizard requires this dependency`)
  }
}
...
export async function runOnboarding(
  ...
  try {
    ...
    process.stderr.write(`[${COMMAND_NAMESPACE}] Onboarding wizard unavailable: ${err instanceof Error ? err.message : String(err)}\n`)
    return
  }
  ...
  const llmResult = await runStep(p, 'LLM setup failed', () => runLlmStep(p, pc, authStorage), {
    cancelMessage: `Setup cancelled — you can run /${COMMAND_NAMESPACE} onboarding --resume later.`,
    errorInfo: `You can configure your LLM provider later with /login inside ${BRAND_NAME}.`,
  })
  ...
  if (llmConfigured) {
    ...
    summaryLines.push(`${pc.yellow('↷')} LLM provider: skipped — use /login inside ${BRAND_NAME}`)
  }

  if (searchConfigured) {
    summaryLines.push(`${pc.green('✓')} Web search: ${searchConfigured}`)
  } else {
    summaryLines.push(`${pc.dim('↷')} Web search: not configured — use /search-provider inside ${BRAND_NAME}`)
  }

  if (remoteConfigured) {
    summaryLines.push(`${pc.green('✓')} Remote questions: ${remoteConfigured}`)
  } else {
    summaryLines.push(`${pc.dim('↷')} Remote questions: not configured — use /${COMMAND_NAMESPACE} remote inside ${BRAND_NAME}`)
  }
...
export async function runLlmStep(p: ClackModule, pc: PicoModule, authStorage: AuthStorage): Promise<boolean> {
  ...
  authOptions.push(
    ...
    { value: 'skip', label: 'Skip for now', hint: `use /login inside ${BRAND_NAME} later` },
  )
...
async function runApiKeyFlow(
  ...
  if (providerId === 'openrouter') {
    p.log.info(`Use ${pc.cyan('/model')} inside ${BRAND_NAME} to pick an OpenRouter model.`)
...
export async function runWebSearchStep(
  ...
  options.push(
    ...
    { value: 'skip', label: 'Skip for now', hint: `use /search-provider inside ${BRAND_NAME} later` },
  )
...
export async function runRemoteQuestionsStep(
  ...
  options.push(
    ...
    { value: 'skip', label: 'Skip for now', hint: `use /${COMMAND_NAMESPACE} remote inside ${BRAND_NAME} later` },
  )

  const choice = await p.select({
    message: `Set up remote questions? (get notified when ${BRAND_NAME} needs input)`,
    options,
  })
  ...
  if (choice === 'telegram') {
    ...
    try {
      const res = await fetch(`https://api.telegram.org/bot${trimmed}/sendMessage`, {
        ...
        body: JSON.stringify({ chat_id: trimmedChatId, text: `${BRAND_NAME} remote questions connected.` }),
        signal: AbortSignal.timeout(15_000),
      })
      const data = await res.json() as TelegramGetMeResponse
      if (!data?.ok) {
...
async function runDiscordChannelStep(p: ClackModule, pc: PicoModule, token: string): Promise<string | null> {
  ...
  try {
    ...
    p.log.warn(`Could not fetch Discord servers — configure channel later with /${COMMAND_NAMESPACE} remote discord`)
    return null
  }

  if (guilds.length === 0) {
    p.log.warn(`Bot is not in any Discord servers — configure channel later with /${COMMAND_NAMESPACE} remote discord`)
    return null
  }
  ...
  try {
    ...
    p.log.warn(`Could not fetch channels — configure later with /${COMMAND_NAMESPACE} remote discord`)
    return null
  }

  if (channels.length === 0) {
    p.log.warn(`No text channels found — configure later with /${COMMAND_NAMESPACE} remote discord`)
    return null
  }
  ...
  const channelChoice = await p.select({
    message: `Which channel should ${BRAND_NAME} use for remote questions?`,
    options: [

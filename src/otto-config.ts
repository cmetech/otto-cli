/**
 * OTTO services config — gateway + langflow.
 *
 ...
 * the otto extension's session_start probe) keep reading process.env so
...
/**
 * Resolve <configDir>/config.json (e.g. ~/.otto/config.json). Honors OTTO_HOME
...
export function loadConfig(): OttoConfig {
  ...
  const r = raw as Partial<OttoConfig>

  const gw = (r.gateway as Partial<OttoConfig["gateway"]>) ?? {}
  const lf = (r.langflow as Partial<OttoConfig["langflow"]>) ?? {}
...
function cloneDefault(): OttoConfig {
  return {
...
/**
 * Populate process.env from a OttoConfig for any env var that is currently
...
export function applyConfigToEnv(cfg: OttoConfig): void {
  ...
  if (!process.env.OTTO_LANGFLOW_DISABLED?.trim() && cfg.langflow.enabled === false) {

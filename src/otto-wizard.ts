/**
 * OTTO services first-run wizard.
 *
 ...
 * than refusing to save — users frequently configure OTTO before the
 ...
 */
export async function runOttoWizard(opts?: WizardOptions): Promise<OttoConfig | null> {
  ...
  if (!p) {
    process.stderr.write(
      ...
      `[${COMMAND_NAMESPACE}] Set OTTO_GATEWAY_URL and LANGFLOW_SERVER_URL env vars instead.\n`,
    )
    return null
  }
  ...
  p.log.info(dim(`Env vars (OTTO_GATEWAY_URL etc.) always override this file.`))
...
/**
 * Return true if the OTTO services wizard should run on first launch.
 * Mirrors src/onboarding.ts:shouldRunOnboarding shape.
 */
export function shouldRunOTTOWizard(opts: { isPrint: boolean; isTTY: boolean }): boolean {

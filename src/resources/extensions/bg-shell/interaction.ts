...
export async function queryShellEnv(
	...
	const sentinel = `__OTTO_ENV_${randomUUID().slice(0, 8)}__`;
...
export async function runOnSession(
	...
	const startMarker = `__OTTO_SENTINEL_${sentinel}_START__`;
	const endMarker = `__OTTO_SENTINEL_${sentinel}_END__`;
	const exitVar = `__OTTO_EXIT_${sentinel}__`;

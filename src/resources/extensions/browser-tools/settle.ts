...
export async function settleAfterActionAdaptive(
	...
	await ensureMutationCounter(p).catch((e) => { if ((process.env.OTTO_DEBUG ?? process.env.OTTO_DEBUG)) console.error("[browser-tools] ensureMutationCounter failed:", e.message); });

...
export async function settleAfterActionAdaptive(
	...
	await ensureMutationCounter(p).catch((e) => { if ((process.env.LOOP24_DEBUG ?? process.env.GSD_DEBUG)) console.error("[browser-tools] ensureMutationCounter failed:", e.message); });

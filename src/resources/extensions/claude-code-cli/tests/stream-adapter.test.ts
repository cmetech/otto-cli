...
describe("stream-adapter — session persistence (#2859)", () => {
	...
	test("buildSdkOptions enables context-1m beta for opus-4-8 (#176)", () => {
	...
	test("buildSdkOptions maps reasoning to effort for opus-4-8 (#176)", () => {
		const options = buildSdkOptions("claude-opus-4-8", "test", undefined, { reasoning: "high" });
		assert.equal(options.effort, "high");
	});

	test("buildSdkOptions passes xhigh reasoning natively for opus-4-8 (#176)", () => {

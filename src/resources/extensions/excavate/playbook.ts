...
function stageCheckpointInstructions(workspace: string, gitMode: "git" | "no-git"): string {
  if (gitMode === "git") {
    return [
      "## Checkpointing",
      "- Git is available. Initialize a standalone Git repository in the excavation workspace.",
      "- After each stage or worker batch, run `git add -A` and commit with the requested commit message.",
      "- These commits are local to the excavation workspace and are separate from the target repository.",
    ].join("\n");
  }

  return [
    "## Checkpointing",
    "- Git is NOT available. Do not run `git init`, `git add`, `git commit`, or any other Git command.",
    `- After each stage or worker batch, append a JSON line to \`${workspace}/provenance/stage-log.jsonl\` instead of committing.`,
    "- Each JSON line MUST include: `ts`, `stage`, `status`, `message`, and `artifacts`.",
    "- Continue writing all excavation artifacts normally; only Git checkpoint commits are disabled.",
  ].join("\n");
}

function checkpointVerb(gitMode: "git" | "no-git", message: string): string {
  if (gitMode === "git") return `Commit: \`${message}\`.`;
  return `Append a checkpoint entry with message \`${message}\` to \`provenance/stage-log.jsonl\`.`;
}

function checkpointSentence(gitMode: "git" | "no-git"): string {
  if (gitMode === "git") return "Commit after the batch.";
  return "Append a checkpoint entry to `provenance/stage-log.jsonl` after the batch.";
}
 ...
 */
export function buildPlaybook({ target, workspace, skillPaths, gitMode = "git" }: PlaybookInput): string {
...
- When workers in a stage are independent, dispatch them in ONE message in parallel, but cap each batch at 4 workers. Wait for the batch, checkpoint, then the next batch.
...
${stageCheckpointInstructions(workspace, gitMode)}
...
${gitMode === "git"
  ...
  : `printf '%s\\n' '{"ts":"<now>","stage":"init","status":"started","message":"initialized no-git excavation workspace","artifacts":["workspace.json"]}' >> "$WS"/provenance/stage-log.jsonl`
}
\`\`\`
Write \`$WS/workspace.json\` with { target: "${target}", created_at: <now>, tier: "raw", git_mode: "${gitMode}", stages: [] }. ${checkpointVerb(gitMode, `[init] workspace for ${target}`)}

## Stage 1 — Source analysis
Dispatch ROLE source-mapper → Read \`${skillPaths["excavate-source-analysis"]}\`. Read every source file under \`${target}\` (do not skim). Decompose into modules; write per-area source notes to \`${workspace}/raw/source/\` and a module list to \`${workspace}/raw/synthesis/module-map.md\` (one \`### <module>\` heading per module). DoD: module-map.md has ≥1 module and source notes exist. ${checkpointVerb(gitMode, "[stage-1] source analysis")}

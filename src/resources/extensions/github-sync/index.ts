...
async function showStatus(ctx: import("@otto/pi-coding-agent").ExtensionCommandContext) {
  if (!ghIsAvailable()) {
...
async function runBootstrap(ctx: import("@otto/pi-coding-agent").ExtensionCommandContext) {
  if (!ghIsAvailable()) {
    ctx.ui.notify("GitHub sync: `gh` CLI not installed or not authenticated.", "warning");
    return;
  }

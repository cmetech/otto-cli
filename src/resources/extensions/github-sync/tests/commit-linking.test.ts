...
import { buildTaskCommitMessage } from "../../workflow/git-service.ts";
...
describe("commit linking", () => {
  it("appends Resolves #N when issueNumber is set", () => {
    ...
    assert.ok(msg.includes("OTTO-Task: S01/T02"), "OTTO-Task trailer present");
  });
  ...
  it("includes both key files and Resolves #N", () => {
    ...
    assert.ok(msg.includes("OTTO-Task: S01/T02"), "OTTO-Task trailer present");
    // OTTO-Task should come after key files but before Resolves
    const keyFilesIdx = msg.indexOf("- src/auth.ts");
    const taskIdx = msg.indexOf("OTTO-Task: S01/T02");
    const resolvesIdx = msg.indexOf("Resolves #43");
    assert.ok(taskIdx > keyFilesIdx, "OTTO-Task after key files");
    assert.ok(resolvesIdx > taskIdx, "Resolves after OTTO-Task");
  });
  ...
  it("no Resolves trailer when issueNumber is not set", () => {
    ...
    assert.ok(msg.includes("OTTO-Task: S01/T02"), "OTTO-Task trailer still present");
  });
});

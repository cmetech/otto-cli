// OTTO — Ollama Extension: First-class local LLM support
/**
 * Ollama Extension
 *
...
export async function probeAndRegister(pi: ExtensionAPI): Promise<boolean> {
	...
	if (models.length === 0) {
		// No local models means there's nothing usable to register in the workflow.
...
export default function ollama(pi: ExtensionAPI) {
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		// Surface probe failures under OTTO_DEBUG so users can diagnose silent
		...
		const debugOllama = (where: string, error: unknown): void => {
			if ((process.env.OTTO_DEBUG ?? process.env.OTTO_DEBUG)) {

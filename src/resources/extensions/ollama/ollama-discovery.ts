// LOOP24 — Ollama model discovery and capability detection
...
async function enrichModel(info: OllamaModelInfo, deps: ClientDeps): Promise<DiscoveredOllamaModel> {
	...
	if (caps.contextWindow === undefined) {
		try {
			...
			if ((process.env.LOOP24_DEBUG ?? process.env.GSD_DEBUG)) console.warn(`[ollama] /api/show failed for ${info.name}:`, err instanceof Error ? err.message : String(err));
		}
	}

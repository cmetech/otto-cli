 ...
 *
 * Compliance: honors OTTO_GATEWAY_URL when set so all LLM traffic exits
 * through the OTTO gateway. Without a gateway, requires ANTHROPIC_API_KEY.
 *
 * Model: defaults to claude-haiku-4-5-20251001 (fast, cheap, ideal for a
 * polish task). Override with OTTO_PROMPT_ENGINEER_MODEL.
 */
...
async function runPromptEngineer(description: string): Promise<PromptEngineerResult> {
  ...
  const baseURL = process.env.OTTO_GATEWAY_URL?.trim() || undefined;
  const gatewayToken = process.env.OTTO_GATEWAY_TOKEN?.trim() || undefined;
  ...
  if (baseURL) {
    ...
    if (!directApiKey) {
      throw new Error(
        "No LLM credentials configured. Set OTTO_GATEWAY_URL (gateway mode) " +
  ...
  const modelId = process.env.OTTO_PROMPT_ENGINEER_MODEL?.trim() || DEFAULT_MODEL;

// OTTO + Durable subagent run status, tracking names, and result artifact store.
...
export function createInitialRunRecord(input: {
	...
	contextMode: "fresh" | "fork";
	cwd: string;
	children: Array<{ agent: string; trackingName?: string; scratchpadName?: string; task: string; cwd?: string }>;
	now?: string;
}): SubagentRunRecord {

...
/**
 * Auto-Worktree -- lifecycle management for auto-mode worktrees.
 *
...
import { WorkflowError, IO_ERROR, GIT_ERROR } from "./errors.js";
import {
  reconcileWorktreeDb,
  isDbAvailable,
  getMilestone,
...
export function mergeMilestoneToMain(
  ...
  if (worktreeCwd !== originalBasePath_) {
    try {
      ...
      // Re-throw WorkflowError (divergence); swallow rev-parse failures
  ...
  if (!mergeResult.success) {
    ...
    if (conflictedFiles.length > 0) {
      // Separate auto-resolvable conflicts (workflow state files + build artifacts)
      // from real code conflicts. workflow state files diverge between branches

...
import { runDoctor, rebuildState } from "./doctor.js";
import {
...
export async function cleanupAfterLoopExit(ctx: ExtensionContext): Promise<void> {
  ...
  // Clear crash lock and release session lock so the next `/otto next` does
...
export async function stopAuto(
  ...
  try {
    ...
    // Drop the active-tool baseline so a subsequent /otto auto run on the
 ...
 * The user can interact with the agent, then `/otto auto` resumes
...
export async function startAuto(
  ...
  if (s.paused) {
    ...
    try {
      const report = await runDoctor(s.basePath, { fix: true });

...
import { SettingsManager, getAgentDir } from "@otto/pi-coding-agent";
...
export async function runClaudeImportFlow(
  ...
  const assetChoice = await ctx.ui.select("Import Claude assets into OTTO/Pi config", [
    "Skills + plugins",
    "Skills only",
    "Plugins only",
    "Cancel",
  ...
  if (importSkills) {
    ...
    if (selectedSkills.length > 0) {
      const prefMode = await ctx.ui.select("How should OTTO treat the imported skills?", [
  ...
  if (importedSkillsCount > 0) {
    ...
    lines.push(`- Skill refs added to OTTO preferences (${scope}) when selected`);
  }

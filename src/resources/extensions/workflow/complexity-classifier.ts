// Workflow Extension — Complexity Classifier
...
function analyzePlanComplexity(
  ...
  const researchPath = join(workflowRoot(basePath), mid, "slices", sid, "RESEARCH.md");
  try {
...
export function extractTaskMetadata(unitId: string, basePath: string): TaskMetadata {
  ...
  const taskPlanPath = join(workflowRoot(basePath), mid, "slices", sid, "tasks", `${tid}-PLAN.md`);

/**
 * Gate Registry — single source of truth for quality-gate ownership.
 *
 ...
 *   - `assertGateCoverage(pending, turn)` throws a WorkflowError if the pending
...
export const GATE_REGISTRY = {
  ...
  Q4: {
    ...
    guidance: [
      "List the R-IDs (e.g. R001, R003) touched by this slice; see the milestone requirements artifact at .otto/workflow/milestones/<id>/REQUIREMENTS.md.",
...
export function getOwnerTurn(id: GateId): OwnerTurn {
  ...
  if (!def) {
    throw new WorkflowError(PARSE_ERROR, `gate-registry: unknown gate id "${id}"`);
  }

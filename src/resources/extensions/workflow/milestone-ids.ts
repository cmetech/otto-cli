 ...
 *
 * Consumed by 15+ modules across the workflow extension. Zero side-effects.
 */
 ...
 * otto_milestone_generate_id (tool) share this set so the ID shown in the UI
...
export function findMilestoneIds(basePath: string): string[] {
  ...
  try {
    ...
    if (existsSync(dir)) {
      logWarning("engine", `findMilestoneIds: .otto/workflow/milestones/ exists but readdirSync failed — ${getErrorMessage(err)}`);
    }
    return [];
  }
}

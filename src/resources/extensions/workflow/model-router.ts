 ...
 * Used to detect Groq at the routing layer where only the provider string
...
export function filterToolsForProvider(
  ...
  if (provider && GROQ_PROVIDER_IDS.has(provider) && compatible.length > GROQ_MAX_TOOLS) {
    ...
    console.warn(
      `[otto] Groq tool limit: ${compatible.length + trimmed.length} tools active but Groq allows at most ${GROQ_MAX_TOOLS}. ` +

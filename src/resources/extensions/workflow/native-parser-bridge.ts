...
let nativeModule: {
  ...
  batchParseWorkflowFiles: (directory: string) => { files: Array<{ path: string; metadata: string; body: string; sections: string; rawContent: string }>; count: number };
  ...
  scanWorkflowTree: (directory: string) => Array<{ path: string; name: string; isDir: boolean }>;
...
export function nativeBatchParseWorkflowFiles(directory: string): BatchParsedFile[] | null {
  ...
  const result = native.batchParseWorkflowFiles(directory);
...
export function nativeScanWorkflowTree(directory: string): WorkflowTreeEntry[] | null {
  const native = loadNative();
  if (!native) return null;
  return native.scanWorkflowTree(directory);
}

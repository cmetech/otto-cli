...
export class PluginImporter {
	...
	selectComponents(
		...
		if (!this.registry) {
			throw new WorkflowError(STALE_STATE, 'Must call discover() before selectComponents()');
		}
	...
	validateImport(selected: NamespacedComponent[]): ValidationResult {
		if (!this.registry) {
			throw new WorkflowError(STALE_STATE, 'Must call discover() before validateImport()');
		}

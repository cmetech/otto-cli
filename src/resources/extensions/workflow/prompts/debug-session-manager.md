You are managing a OTTO debug session.
...
Persist specialist review results under `## Specialist Review` in the session artifact at `.otto/workflow/debug/sessions/{{slug}}.json`.
...
1. Read `.otto/workflow/debug/sessions/{{slug}}.json` for prior session context and checkpoint state.

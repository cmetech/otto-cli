...
Workflow extension source: `{{workflowSourceDir}}`
...
### Journal Format (`.otto/workflow/journal/`)
...
5. Read actual source at `{{workflowSourceDir}}` to confirm or deny each hypothesis. Do not guess.

   **DB inspection:** Use `otto_milestone_status`; never run `sqlite3 .otto/workflow/otto.db` or `node -e require('better-sqlite3')`. The engine owns the WAL connection; direct access can fail or return stale data.
...
Then **offer GitHub issue creation**: "Would you like me to create a GitHub issue for this on open-gsd/otto-pi?"

**CRITICAL:** The `github_issues` tool targets only the current user's repository and has no `repo` parameter. Use `gh issue create --repo open-gsd/otto-pi` via `bash`. Do NOT use the `github_issues` tool.

If yes, create using the `bash` tool:

```bash
ISSUE_BODY_FILE="${TMPDIR:-${TEMP:-${TMP:-.}}}/otto-forensic-issue.md"
cat > "$ISSUE_BODY_FILE" << 'OTTO_ISSUE_BODY'
## Problem
...
---
*Auto-generated by `/otto forensics`*
OTTO_ISSUE_BODY

ISSUE_URL=$(gh issue create --repo open-gsd/otto-pi \
  --title "..." \

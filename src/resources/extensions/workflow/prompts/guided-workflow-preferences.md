**Working directory:** `{{workingDirectory}}`. All file reads, writes, and shell commands MUST operate relative to this directory. Do NOT `cd` to any other directory. For `.otto/workflow` files in this prompt, use absolute paths rooted at `{{workingDirectory}}` instead of discovering them with `Glob`.

Configure project workflow preferences. This stage runs ONCE per project, early in deep-mode bootstrap, before `discuss-project`. It applies a small set of recommended workflow defaults and persists them to the YAML frontmatter of `.otto/workflow/PREFERENCES.md` (the same file the runtime reads its preferences from).
...
1. Read `{{workingDirectory}}/.otto/workflow/PREFERENCES.md` if it exists. The file is YAML frontmatter (between `---` lines) followed by an optional markdown body. Parse the existing frontmatter so you can preserve unrelated keys (e.g. `planning_depth`).
...
4. Write `{{workingDirectory}}/.otto/workflow/PREFERENCES.md` back with the merged frontmatter and the original body preserved unchanged. Frontmatter delimiters are exactly `---` on their own lines.
5. Pre-seed the research decision so the standalone `research-decision` stage is a no-op if the user already answered here:
   - Ensure `{{workingDirectory}}/.otto/workflow/runtime/` exists.
   - Write `{{workingDirectory}}/.otto/workflow/runtime/research-decision.json`:
     ```json
     {
       "decision": "skip",
     ...
     ```
   Use `"skip"` unless an existing valid `{{workingDirectory}}/.otto/workflow/runtime/research-decision.json` explicitly says `"research"` with `"source": "research-decision"` or `"source": "user"`.
...
Do NOT write to `.otto/workflow/config.json`; runtime preferences load from `PREFERENCES.md`.
...
- Do NOT change any keys other than the frontmatter keys specified plus `workflow_prefs_captured`. Research is persisted to `.otto/workflow/runtime/research-decision.json`, NOT to `phases.skip_research`.

...
5. **If a failed task produced no summary, call `otto_summary_save`** with `milestone_id: {{milestoneId}}`, `slice_id: {{sliceId}}`, the failed task's `task_id`, and `artifact_type: "SUMMARY"` — include `blocker_discovered: true` and clear failure details in the `content`. Do NOT call `otto_task_complete` for the failed task — leave it uncompleted so replan/retry has an authoritative record.

...
5. Decompose the slice into tasks that fit one context window each. Every task passed to `otto_plan_slice` must use the exact keys `taskId`, `title`, `description`, `estimate`, `files`, `verify`, `inputs`, `expectedOutput`, and optional `observabilityImpact`. Put Why / Do / Done-when detail in `description`. `files`, `inputs`, and `expectedOutput` must be JSON arrays of strings, even for one path (for example, `"expectedOutput": ["src/index.ts"]`, never `"expectedOutput": "src/index.ts"`).
6. **Persist planning state through `otto_plan_slice`.** Call it with the full payload. The tool writes to the DB and renders `{{outputPath}}` and `{{slicePath}}/tasks/T##-PLAN.md` automatically. Do NOT rely on direct `PLAN.md` writes.
...
8. If refinement produced structural decisions that diverge from the sketch, call `otto_decision_save` for each; the tool persists the decision and regenerates `.otto/workflow/DECISIONS.md`.
9. {{commitInstruction}}
...
**You MUST call `otto_plan_slice` to persist planning state before finishing.** After success, the pipeline clears the sketch flag on next state derivation; the on-disk PLAN file is the signal.

...
export function validateFileChanges(
  ...
  // Filter out .otto/workflow/ internal files — only validate project source files
  const projectFiles = actualFiles.filter(f => !f.startsWith(".otto/workflow/") && !f.startsWith(".otto\\workflow\\"));

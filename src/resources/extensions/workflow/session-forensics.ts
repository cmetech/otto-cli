/**
 * Session Forensics — Deep analysis of pi session JSONL files
 *
 ...
 * - Stuck-retry diagnostics (reading activity log copies)
 *
 ...
 * Works with both pi session JSONL and activity log JSONL.
 */
 ...
 *
 * Reads the surviving pi session file (or falls back to the last activity
...
export function getDeepDiagnostic(basePath: string, worktreePath?: string): string | null {
  ...
  try {
    if (worktreePath) {
      const wtActivityDir = join(workflowRoot(worktreePath), "activity");
...
export function readActiveMilestoneId(basePath: string): string | null {
  try {
    const statePath = join(workflowRoot(basePath), "STATE.md");

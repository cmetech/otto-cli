---
name: gsd-headless
description: Orchestrate OTTO projects programmatically via headless CLI. Use when an agent needs to create milestones from specs, execute software development workflows, monitor task progress, check project status, or control OTTO execution (pause/stop/skip/steer). Triggers on requests to "run otto", "create milestone", "execute project", "check otto status", "orchestrate development", "run headless workflow", or any programmatic interaction with the OTTO project management system. Essential for building orchestrators that coordinate multiple OTTO workers.
---

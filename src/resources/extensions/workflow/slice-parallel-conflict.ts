/**
 * Slice Parallel Conflict Detection — File overlap analysis between slices.
 *
...
export function hasFileConflict(
  ...
  const planPathA = join(basePath, ".otto/workflow", "milestones", mid, "slices", sliceA, `${sliceA}-PLAN.md`);
  const planPathB = join(basePath, ".otto/workflow", "milestones", mid, "slices", sliceB, `${sliceB}-PLAN.md`);

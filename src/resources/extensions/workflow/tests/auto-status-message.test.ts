// Project/App: OTTO
// File Purpose: Visual contract tests for compact OTTO status notification cards.
...
test("formatConnectedStepStack renders task completion as a compact receipt", () => {
  ...
  assert.match(plain, /^    ╭─ ✓ OTTO Task Complete/m);
  ...
  assert.doesNotMatch(plain, /Continue: \/otto next/);
  assert.doesNotMatch(plain, /Auto-run: \/otto auto/);
  assert.doesNotMatch(plain, /\/otto next/);
  assert.doesNotMatch(plain, /\/otto auto/);
  assert.doesNotMatch(plain, /^          ╭─/m);
  assert.doesNotMatch(plain, /│/);
});

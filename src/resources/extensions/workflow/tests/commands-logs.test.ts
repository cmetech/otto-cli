...
function createTestDir(): string {
  ...
  mkdirSync(join(dir, ".otto/workflow", "debug"), { recursive: true });
  return dir;
}
...
function writeActivityLog(dir: string, seq: number, unitType: string, unitId: string, entries: Record<string, unknown>[]): void {
  ...
  writeFileSync(join(dir, ".otto/workflow", "activity", filename), content);
}

function writeDebugLog(dir: string, name: string, entries: Record<string, unknown>[]): void {
  const content = entries.map(e => JSON.stringify(e)).join("\n") + "\n";
  writeFileSync(join(dir, ".otto/workflow", "debug", name), content);
}
...
test("logs lists activity logs", async (t) => {
  ...
  assert.ok(msg.includes("/otto logs <#>"), "should show usage hint");
});
...
test("logs clear removes old logs", async (t) => {
  ...
  const oldFile = join(dir, ".otto/workflow", "activity", "001-execute-task-M001-S01-T01.jsonl");
  ...
  assert.ok(
    existsSync(join(dir, ".otto/workflow", "activity", "007-execute-task-M001-S01-T07.jsonl")),

...
test("complete milestone prompt renders compact verification and completion guidance", async (t) => {
  ...
  const isolatedHome = providedWorkflowHome ?? mkdtempSync(join(tmpdir(), "gsd-complete-milestone-render-"));
  ...
  const prompt = loadPrompt("complete-milestone", {
    ...
    milestoneSummaryPath: ".otto/workflow/milestones/M001/M001-SUMMARY.md",

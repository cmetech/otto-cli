...
/**
 * Create a temp project directory with .otto/workflow structure for handler tests.
 */
function createTempProject(): { basePath: string; planPath: string } {
  const basePath = fs.mkdtempSync(path.join(os.tmpdir(), 'gsd-handler-'));
  const tasksDir = path.join(basePath, '.otto/workflow', 'milestones', 'M001', 'slices', 'S01', 'tasks');
  fs.mkdirSync(tasksDir, { recursive: true });

  const planPath = path.join(basePath, '.otto/workflow', 'milestones', 'M001', 'slices', 'S01', 'S01-PLAN.md');
...
console.log('\n=== complete-task: accessor stale-state error ===');
{
  // No DB open — accessors should throw STALE_STATE
  closeDatabase();
  let threw = false;
  try {
    ...
    threw = true;
    assertTrue(err.code === 'STALE_STATE' || err.message.includes('No database open'),
      'should throw STALE_STATE when no DB open');
  }
  ...
  try {
    ...
    threw = true;
    assertTrue(err.code === 'STALE_STATE' || err.message.includes('No database open'),
      'insertSlice should throw STALE_STATE');
  }
  ...
  try {
    ...
    threw = true;
    assertTrue(err.code === 'STALE_STATE' || err.message.includes('No database open'),
      'insertTask should throw STALE_STATE');
  }
  ...
  try {
    ...
    threw = true;
    assertTrue(err.code === 'STALE_STATE' || err.message.includes('No database open'),
      'insertVerificationEvidence should throw STALE_STATE');
  }

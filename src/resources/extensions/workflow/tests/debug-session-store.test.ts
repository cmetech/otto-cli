...
describe("debug-session-store: create/list/load/update", () => {
  test("creates first session under .otto/workflow/debug/sessions with deterministic metadata", () => {
    const base = makeBase();
    try {
      ...
      assert.ok(created.artifactPath.includes(join(".otto/workflow", "debug", "sessions")));
      assert.ok(created.artifactPath.endsWith("login-fails-on-safari.json"));
      assert.ok(created.session.logPath.includes(join(".otto/workflow", "debug")));
  ...
  test("backward compat: existing artifact without checkpoint/tddGate fields validates successfully", () => {
    ...
    try {
      ...
      const artifact = {
        ...
        createdAt: 1000,
        updatedAt: 1000,
        logPath: join(base, ".otto/workflow", "debug", "legacy-session.log"),
        lastError: null,
      };
  ...
  test("validator rejects malformed checkpoint — missing required sub-fields", () => {
    ...
    try {
      ...
      const artifact = {
        ...
        createdAt: 1000,
        updatedAt: 1000,
        logPath: join(base, ".otto/workflow", "debug", "bad-checkpoint.log"),
        lastError: null,
  ...
  test("validator rejects malformed tddGate — missing required sub-fields", () => {
    ...
    try {
      ...
      const artifact = {
        ...
        createdAt: 1000,
        updatedAt: 1000,
        logPath: join(base, ".otto/workflow", "debug", "bad-tddgate.log"),
        lastError: null,
  ...
  test("backward compat: existing artifact without specialistReview validates successfully", () => {
    ...
    try {
      ...
      const artifact = {
        ...
        createdAt: 1000,
        updatedAt: 1000,
        logPath: join(base, ".otto/workflow", "debug", "legacy-no-specialist.log"),
        lastError: null,
      };
  ...
  test("validator rejects specialistReview with missing required fields (empty object)", () => {
    ...
    try {
      ...
      const artifact = {
        ...
        createdAt: 1000,
        updatedAt: 1000,
        logPath: join(base, ".otto/workflow", "debug", "bad-specialist-empty.log"),
        lastError: null,
        specialistReview: {},
      };
  ...
  test("validator rejects specialistReview with wrong field types (verdict as number, skill as number)", () => {
    ...
    try {
      ...
      const artifact = {
        ...
        createdAt: 1000,
        updatedAt: 1000,
        logPath: join(base, ".otto/workflow", "debug", "bad-specialist-types.log"),
        lastError: null,
        specialistReview: {
  ...
  test("validator accepts specialistReview with extra unknown fields (forward compat)", () => {
    ...
    try {
      ...
      const artifact = {
        ...
        createdAt: 1000,
        updatedAt: 1000,
        logPath: join(base, ".otto/workflow", "debug", "specialist-extra-fields.log"),
        lastError: null,
        specialistReview: {

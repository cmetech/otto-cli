// the agent — Behavioural regression test for #3616.
//
...
// otto_plan_slice and other DB tools to be missing from plan-slice subagent
// sessions.
//
// The behavioural invariant we can pin without grepping source: otto_plan_slice
...
describe("#3616 — discuss tool scoping must not leak across sessions", () => {
	test("otto_plan_slice is NOT in DISCUSS_TOOLS_ALLOWLIST", () => {
		assert.ok(
			!DISCUSS_TOOLS_ALLOWLIST.includes("otto_plan_slice"),
			`otto_plan_slice (a heavy planning tool) must be excluded from the discuss scope; allowlist=${JSON.stringify(DISCUSS_TOOLS_ALLOWLIST)}`,
		);
	});
	...
	test("DISCUSS_TOOLS_ALLOWLIST is non-empty (sanity)", () => {
		assert.ok(
			...
			"discuss scope should include at least one tool — empty allowlist would break /otto discuss",
		);
	});
});

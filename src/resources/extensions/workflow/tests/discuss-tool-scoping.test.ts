 ...
 * schemas exceed the provider's grammar limit. The OTTO discuss flow only
...
const DISCUSS_REQUIRED_TOOLS = [
  "otto_summary_save",          // guided-discuss-slice.md, guided-discuss-milestone.md, discuss.md
  "otto_decision_save",         // discuss.md output phase
  "otto_plan_milestone",        // discuss.md output phase (single + multi milestone)
  "otto_milestone_generate_id", // discuss.md multi-milestone Phase 1
  "otto_requirement_save",      // discuss-headless.md capability contract creation
  "otto_requirement_update",    // used during discuss for requirement updates
];
...
describe("discuss tool scoping (#2949)", () => {
  ...
  test("DISCUSS_TOOLS_ALLOWLIST is significantly smaller than full tool set", () => {
    ...
    // Discuss set should be roughly 10 OTTO tools (5 canonical + 5 aliases)
    assert.ok(
      DISCUSS_TOOLS_ALLOWLIST.length <= 12,
      `allowlist should have at most 12 OTTO tools, got ${DISCUSS_TOOLS_ALLOWLIST.length}`,
    );
  });
  ...
  test("dispatchWorkflow scopes and restores tools for discuss unit types", async () => {
    ...
    let activeTools = ["otto_summary_save", "otto_complete_task", "bash"];
    ...
    try {
      ...
      if (originalWorkflowPath === undefined) delete process.env.OTTO_WORKFLOW_PATH;
      else process.env.OTTO_WORKFLOW_PATH = originalWorkflowPath;
      rmSync(tmp, { recursive: true, force: true });
    }

    assert.deepEqual(setCalls[0], ["otto_summary_save", "bash"]);
    assert.deepEqual(setCalls.at(-1), ["otto_summary_save", "otto_complete_task", "bash"]);
    assert.ok(sentTools.length > 0, "dispatch should queue a message");
    assert.ok(sentTools.includes("otto_summary_save"), "dispatch keeps the discuss save tool");
    assert.ok(!sentTools.includes("otto_complete_task"), "dispatch removes heavy completion tools");

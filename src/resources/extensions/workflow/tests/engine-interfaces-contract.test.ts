...
describe("Leaf-node constraint", () => {
  test("engine-types.ts has zero imports from OTTO modules (only node: allowed)", () => {
    ...
    assert.equal(
      violations.length,
      0,
      `engine-types.ts must be a leaf node with zero OTTO imports. ` +
...
describe("EngineState shape", () => {
  test("EngineState has all required fields with correct types", () => {
    ...
    // raw must be typed unknown — not a OTTO-specific type
    assert.ok(
      /raw:\s*unknown/.test(source),
      "EngineState.raw must be typed 'unknown', not a OTTO-specific type",
    );
  });
});

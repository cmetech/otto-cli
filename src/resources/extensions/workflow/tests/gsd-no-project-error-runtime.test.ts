/**
 * Runtime regression — `projectRoot()` throws `NoProjectError` when
 ...
 * asserts a `NoProjectError` is thrown with the project-required
 * message.
 */
...
import { projectRoot, NoProjectError, withCommandCwd } from '../commands/context.ts';
...
describe('projectRoot() throws NoProjectError outside a project (#4902)', () => {
  test('throws NoProjectError when cwd is $HOME', () => {
    ...
    try {
      assert.throws(
        () => projectRoot(),
        (err: unknown) => {
          assert.ok(err instanceof NoProjectError, 'should throw NoProjectError');
          assert.match(
  ...
  test('throws NoProjectError when cwd is the system tmpdir root', () => {
    ...
    try {
      // Behaviour: either we get a NoProjectError (blocked tmpdir root) or
      ...
      if (threw !== null) {
        assert.ok(
          threw instanceof NoProjectError,
          'if projectRoot throws, it must be a NoProjectError (typed)',
        );
      }
    } finally {
      process.chdir(ORIGINAL_CWD);
...
describe('NoProjectError shape (#4902)', () => {
  test('NoProjectError extends Error and carries its name', () => {
    const err = new NoProjectError('test reason');
    assert.ok(err instanceof Error);
    assert.equal(err.name, 'NoProjectError');

...
test("guided project prompt renders compact interview and artifact guidance", async (t) => {
  ...
  const isolatedHome = providedWorkflowHome ?? mkdtempSync(join(tmpdir(), "gsd-guided-project-render-"));
  ...
  assert.doesNotMatch(prompt, /then write `.otto\/workflow\/PROJECT\.md`/);

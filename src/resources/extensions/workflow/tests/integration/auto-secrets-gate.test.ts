 ...
 *
 * Uses temp directories with real .otto/workflow/milestones/M001/ structure, mirroring
...
/** Create the .otto/workflow/milestones/M001/ directory structure and write a secrets manifest. */
function writeManifest(base: string, content: string): void {
  const mDir = join(base, '.otto/workflow', 'milestones', 'M001');
...
test('secrets gate: pending keys exist — gate triggers collection, manifest updated on disk', async (t) => {
  ...
  const savedA = process.env.OTTO_GATE_TEST_EXISTING;
  t.after(() => {
    delete process.env.OTTO_GATE_TEST_EXISTING;
    if (savedA !== undefined) process.env.OTTO_GATE_TEST_EXISTING = savedA;
    delete process.env.OTTO_GATE_TEST_PEND_A;
    delete process.env.OTTO_GATE_TEST_PEND_B;
    rmSync(tmp, { recursive: true, force: true });
  });

  // Simulate one key already in env
  process.env.OTTO_GATE_TEST_EXISTING = 'already-here';
...
`);
  ...
  assert.deepStrictEqual(status!.pending, ['OTTO_GATE_TEST_PEND_A', 'OTTO_GATE_TEST_PEND_B'], 'pending keys');
  assert.deepStrictEqual(status!.existing, ['OTTO_GATE_TEST_EXISTING'], 'existing keys');
  ...
  assert.ok(result.skipped.includes('OTTO_GATE_TEST_PEND_A'), 'PEND_A should be skipped');
  assert.ok(result.skipped.includes('OTTO_GATE_TEST_PEND_B'), 'PEND_B should be skipped');
  assert.deepStrictEqual(result.existingSkipped, ['OTTO_GATE_TEST_EXISTING']);
  ...
  const manifestPath = join(tmp, '.otto/workflow', 'milestones', 'M001', 'M001-SECRETS.md');
...
test('secrets gate: no pending keys — getManifestStatus shows pending.length === 0', async (t) => {
  ...
  const savedKey = process.env.OTTO_GATE_TEST_ENVKEY;
  t.after(() => {
    delete process.env.OTTO_GATE_TEST_ENVKEY;
    if (savedKey !== undefined) process.env.OTTO_GATE_TEST_ENVKEY = savedKey;
    rmSync(tmp, { recursive: true, force: true });
  });

  process.env.OTTO_GATE_TEST_ENVKEY = 'some-value';
...
`);
  ...
  assert.deepStrictEqual(result!.existing, ['OTTO_GATE_TEST_ENVKEY']);
});

...
describe('git-locale', async () => {
  ...
  test('nativeAddAllWithExclusions: non-English locale does not throw', () => {
    // Simulate what happens on a German system: .otto/workflow is gitignored,
    ...
    assert.ok(
      !threw,
      "nativeAddAllWithExclusions must not throw on non-English locale when .otto/workflow is gitignored (#1997)"
    );

...
describe('git-service', async () => {
  ...
  test('buildTaskCommitMessage', () => {
    ...
    assert.ok(!msg.includes("(S01/T02)"), "no OTTO ID in subject line");
    ...
    assert.ok(msg.includes("OTTO context:"), "message includes human OTTO context block");
    ...
    assert.ok(msg.includes("OTTO-Task: S01/T02"), "OTTO-Task trailer in body");
...

  {
    ...
    assert.ok(msg.includes("OTTO-Task: S02/T01"), "OTTO-Task trailer present");
  }

  {
    ...
    assert.ok(msg.includes("OTTO-Task: S01/T03"), "OTTO-Task trailer present");
  }
  ...
  assert.ok(
    RUNTIME_EXCLUSION_PATHS.includes(".otto/workflow/activity/"),
    "includes .otto/workflow/activity/"
  );
  assert.ok(
    RUNTIME_EXCLUSION_PATHS.includes(".otto/workflow/STATE.md"),
    "includes .otto/workflow/STATE.md"
  );
  ...
  test('GitServiceImpl: smart staging', () => {
    ...
    createFile(repo, ".otto/workflow/activity/log.jsonl", "log data");
    createFile(repo, ".otto/workflow/runtime/state.json", '{"state":true}');
    createFile(repo, ".otto/workflow/STATE.md", "# State");
    createFile(repo, ".otto/workflow/auto.lock", "lock");
    createFile(repo, ".otto/workflow/metrics.json", "{}");
    createFile(repo, ".otto/workflow/worktrees/wt/file.txt", "wt data");
    ...
    assert.ok(!showStat.includes(".otto/workflow/activity"), ".otto/workflow/activity/ excluded from commit");
    assert.ok(!showStat.includes(".otto/workflow/runtime"), ".otto/workflow/runtime/ excluded from commit");
    assert.ok(!showStat.includes("STATE.md"), ".otto/workflow/STATE.md excluded from commit");
    assert.ok(!showStat.includes("auto.lock"), ".otto/workflow/auto.lock excluded from commit");
    assert.ok(!showStat.includes("metrics.json"), ".otto/workflow/metrics.json excluded from commit");
    assert.ok(!showStat.includes(".otto/workflow/worktrees"), ".otto/workflow/worktrees/ excluded from commit");

    // Verify runtime files are still untracked
    // git status --short may collapse to "?? .otto/workflow/" or show individual files
    // Use --untracked-files=all to force individual listing
    const statusOut = run("git status --short --untracked-files=all", repo);
    assert.ok(statusOut.includes(".otto/workflow/activity/"), "activity still untracked after commit");
    assert.ok(statusOut.includes(".otto/workflow/runtime/"), "runtime still untracked after commit");
    assert.ok(statusOut.includes(".otto/workflow/STATE.md"), "STATE.md still untracked after commit");
  ...
  test('GitServiceImpl: smart staging excludes tracked runtime files', () => {
    // Reproduces the real bug: .otto/workflow/ runtime files that are already tracked
    // (in the git index) must be excluded from staging even when .otto/workflow/ is
    ...
    // 3. Pre-checkout discard: git checkout -- .otto/workflow/ clears dirty runtime files

    const repo = initTempRepo();
    const svc = new GitServiceImpl(repo);

    // Simulate a repo where .otto/workflow/ files were previously force-added
    createFile(repo, ".otto/workflow/metrics.json", '{"version":1}');
    createFile(repo, ".otto/workflow/completed-units.json", '["unit1"]');
    createFile(repo, ".otto/workflow/activity/log.jsonl", '{"ts":1}');
    createFile(repo, "src/real.ts", "real code");
    // Force-add .otto/workflow/ files to simulate historical tracking
    runGit(repo, ["add", "-f", ".otto/workflow/metrics.json", ".otto/workflow/completed-units.json", ".otto/workflow/activity/log.jsonl", "src/real.ts"]);
    runGit(repo, ["commit", "-F", "-"], { input: "init with tracked runtime files" });

    // Add .gitignore with .otto/workflow/ (matches real-world setup from ensureGitignore)
    createFile(repo, ".gitignore", ".otto/workflow/\n");
    runGit(repo, ["add", ".gitignore"]);
    runGit(repo, ["commit", "-F", "-"], { input: "add gitignore" });

    // Verify runtime files are tracked (precondition)
    const tracked = run("git ls-files .otto/workflow/", repo);
    ...
    createFile(repo, ".otto/workflow/metrics.json", '{"version":2}');
    createFile(repo, ".otto/workflow/completed-units.json", '["unit1","unit2"]');
    createFile(repo, ".otto/workflow/activity/log.jsonl", '{"ts":2}');
    ...
    const trackedAfter = run("git ls-files .otto/workflow/", repo);
    assert.deepStrictEqual(trackedAfter, "", "no .otto/workflow/ runtime files remain in the index");

    // Verify a second autoCommit with changed runtime files does NOT stage them
    createFile(repo, ".otto/workflow/metrics.json", '{"version":3}');
    createFile(repo, ".otto/workflow/completed-units.json", '["unit1","unit2","unit3"]');
  ...
  test('GitServiceImpl: autoCommit on dirty repo', () => {
    ...
    assert.deepStrictEqual(msg, "chore: auto-commit after task\n\nOTTO-Unit: T01", "autoCommit returns generic format with trailer");
    ...
    assert.ok(msg2!.includes("OTTO-Task: S01/T02"), "meaningful commit has OTTO-Task trailer");
  ...
  test('GitServiceImpl: empty-after-staging guard', () => {
    ...
    createFile(repo, ".otto/workflow/activity/x.jsonl", "data");
  ...
  test('GitServiceImpl: autoCommit with extraExclusions', () => {
    ...
    // Create both a .otto/workflow/ planning file and a regular source file
    createFile(repo, ".otto/workflow/milestones/M001/M001-ROADMAP.md", "- [x] S01");
    createFile(repo, "src/feature.ts", "export const y = 2;");

    // Auto-commit with .otto/workflow/ excluded (simulates pre-switch)
    const msg = svc.autoCommit("pre-switch", "main", [".otto/workflow/"]);
    assert.deepStrictEqual(msg, "chore: auto-commit after pre-switch\n\nOTTO-Unit: main", "pre-switch autoCommit with .otto/workflow/ exclusion commits");

    // Verify .otto/workflow/ file was NOT committed
    const show = run("git show --stat HEAD", repo);
    assert.ok(!show.includes("ROADMAP"), ".otto/workflow/ files excluded from pre-switch auto-commit");
    assert.ok(show.includes("feature.ts"), "non-.otto/workflow/ files included in pre-switch auto-commit");

    rmSync(repo, { recursive: true, force: true });
  });

  // ─── GitServiceImpl: autoCommit extraExclusions — only .otto/workflow/ dirty ────

  test('GitServiceImpl: autoCommit extraExclusions — only .otto/workflow/ dirty', () => {
    const repo = initTempRepo();
    const svc = new GitServiceImpl(repo);

    // Create only .otto/workflow/ planning files
    createFile(repo, ".otto/workflow/milestones/M001/M001-ROADMAP.md", "- [x] S01");
    createFile(repo, ".otto/workflow/STATE.md", "state content");

    // Auto-commit with .otto/workflow/ excluded — nothing else to commit
    const result = svc.autoCommit("pre-switch", "main", [".otto/workflow/"]);
    assert.deepStrictEqual(result, null, "autoCommit returns null when only .otto/workflow/ files are dirty and excluded");
  ...
  test('createSnapshot: enabled by default when prefs omitted', () => {
    ...
    // Verify ref exists under refs/otto/snapshots/
    const refs = run("git for-each-ref refs/otto/snapshots/", repo);
    assert.ok(refs.includes("refs/otto/snapshots/otto/M001/S01/"), "snapshot ref created under refs/otto/snapshots/");
  ...
  test('Integration branch: records non-ephemeral gsd branches', () => {
    ...
    // they match a OTTO-generated workflow template prefix.
  ...
  test('untrackRuntimeFiles', async () => {
    ...
    mkdirSync(join(repo, ".otto/workflow", "activity"), { recursive: true });
    mkdirSync(join(repo, ".otto/workflow", "runtime"), { recursive: true });
    writeFileSync(join(repo, ".otto/workflow", "completed-units.json"), '["u1"]');
    writeFileSync(join(repo, ".otto/workflow", "metrics.json"), '{}');
    writeFileSync(join(repo, ".otto/workflow", "STATE.md"), "# State");
    writeFileSync(join(repo, ".otto/workflow", "activity", "log.jsonl"), "{}");
    writeFileSync(join(repo, ".otto/workflow", "runtime", "data.json"), "{}");
    writeFileSync(join(repo, "src.ts"), "code");
    runGit(repo, ["add", "-A"]);
    runGit(repo, ["commit", "-m", "init"]);

    // Precondition: runtime files are tracked
    const trackedBefore = run("git ls-files .otto/workflow/", repo);
    ...
    assert.ok(existsSync(join(repo, ".otto/workflow", "completed-units.json")),
      "untrack: completed-units.json still on disk");
    assert.ok(existsSync(join(repo, ".otto/workflow", "metrics.json")),
  ...
  test('smartStage excludes runtime files, allows milestone artifacts', () => {
    ...
    // Create .otto/workflow/ runtime files + milestone artifacts + a normal source file
    mkdirSync(join(repo, ".otto/workflow", "milestones", "M001"), { recursive: true });
    mkdirSync(join(repo, ".otto/workflow", "runtime"), { recursive: true });
    mkdirSync(join(repo, ".otto/workflow", "activity"), { recursive: true });
    writeFileSync(join(repo, ".otto/workflow", "milestones", "M001", "ROADMAP.md"), "# Roadmap");
    writeFileSync(join(repo, ".otto/workflow", "PREFERENCES.md"), "---\nversion: 1\n---");
    writeFileSync(join(repo, ".otto/workflow", "STATE.md"), "# State");
    writeFileSync(join(repo, ".otto/workflow", "runtime", "units.json"), "{}");
    writeFileSync(join(repo, ".otto/workflow", "activity", "log.jsonl"), "{}");
    writeFileSync(join(repo, "src.ts"), "const x = 1;");

    // smartStage excludes only runtime paths, not all of .otto/workflow/ (#1326)
    const svc = new GitServiceImpl(repo);
    ...
    assert.ok(!committed.includes(".otto/workflow/STATE.md"), "smartStage: STATE.md excluded (runtime)");
    assert.ok(!committed.includes(".otto/workflow/runtime/"), "smartStage: runtime/ excluded");
    assert.ok(!committed.includes(".otto/workflow/activity/"), "smartStage: activity/ excluded");
    // Milestone artifacts SHOULD be committed when not gitignored (#1326)
    assert.ok(committed.includes(".otto/workflow/milestones/"), "smartStage: milestone artifacts ARE committed");
  ...
  test('writeIntegrationBranch: no commit', () => {
    ...
    // No commit — .otto/workflow/ is managed externally
  ...
  // ─── ensureGitignore: always adds .otto/workflow to gitignore ──────────────────

  test('ensureGitignore: adds .otto/workflow entry', async () => {
    const { ensureGitignore } = await import("../../gitignore.ts");
    const repo = mkdtempSync(join(tmpdir(), "gsd-gitignore-external-state-"));

    // Should add .otto/workflow to gitignore (external state dir is a symlink)
    ...
    assert.ok(lines.includes(".otto/workflow"), "ensureGitignore: .gitignore contains .otto/workflow");
  ...
  // ─── nativeAddAllWithExclusions: symlinked .otto/workflow fallback ───────────────

  test('nativeAddAllWithExclusions: symlinked .otto/workflow fallback', () => {
    // When .otto/workflow is a symlink, git rejects `:!.otto/workflow/...` pathspecs with
    // "fatal: pathspec '...' is beyond a symbolic link". When `.otto/workflow` is
    // already gitignored, the fallback should still stage untracked real files.
    const repo = initTempRepo();

    // Create the real .otto/workflow directory outside the repo, then symlink it
    ...
    // because `.otto/workflow` itself is already protected by .gitignore.
    createFile(repo, "src/new-feature.ts", "export const fresh = true;");

    // nativeAddAllWithExclusions should NOT throw despite .otto/workflow being a symlink
    let threw = false;
    try {
    ...
    assert.ok(!threw, "nativeAddAllWithExclusions does not throw with symlinked .otto/workflow");

    // Verify the tracked modified file was staged
    const staged = run("git diff --cached --name-only", repo);
    assert.ok(staged.includes("src/app.ts"), "modified tracked file staged despite symlinked .otto/workflow");

    assert.ok(staged.includes("src/new-feature.ts"),
      "symlink fallback should still stage new real files when .otto/workflow is gitignored");
    assert.ok(!staged.includes(".otto/workflow"), ".otto/workflow content not staged");

    rmSync(repo, { recursive: true, force: true });
    rmSync(externalGsd, { recursive: true, force: true });
  });

  test('nativeAddAllWithExclusions: self-heals symlinked .otto/workflow when .gitignore lacks it (#4423)', () => {
    // When `.otto/workflow` is a symlink AND not listed in `.gitignore`, the staging
    // fallback must self-heal by appending `.otto/workflow` to `.gitignore` and retrying
    ...
    assert.ok(!threw, "nativeAddAllWithExclusions does not throw with symlinked .otto/workflow when .otto/workflow is not gitignored");

    const staged = run("git diff --cached --name-only", repo);
    assert.ok(staged.includes("src/app.ts"), "tracked modifications stage");
    assert.ok(
      staged.includes("src/new-feature.ts"),
      "self-heal adds .otto/workflow to .gitignore so new user files are staged",
    );
    assert.ok(!staged.includes(".otto/workflow"), ".otto/workflow contents stay unstaged after self-heal");

    // Verify the self-heal actually wrote to .gitignore
    const gitignore = readFileSync(join(repo, ".gitignore"), "utf-8");
    assert.ok(/^\.otto\/workflow\/?$/m.test(gitignore), ".gitignore contains .otto/workflow entry after self-heal");
  ...
  test('nativeAddAllWithExclusions: explicit staging protects work when manage_gitignore:false (#4423)', () => {
    ...
    // by explicitly staging untracked real files while skipping `.otto/workflow`.
    const repo = initTempRepo();
    ...
    assert.ok(!staged.includes(".otto/workflow"), ".otto/workflow contents stay unstaged");
    ...
    if (gitignoreExists) {
      const gitignore = readFileSync(join(repo, ".gitignore"), "utf-8");
      assert.ok(
        !/^\.otto\/workflow\/?$/m.test(gitignore),
  ...
  // ─── nativeAddAllWithExclusions: non-symlinked .otto/workflow still works ───────

  test('nativeAddAllWithExclusions: non-symlinked .otto/workflow still works', () => {
    ...
    assert.ok(!threw, "nativeAddAllWithExclusions works with normal .otto/workflow directory");

    const staged = run("git diff --cached --name-only", repo);
    assert.ok(staged.includes("src/code.ts"), "real file staged with normal .otto/workflow");
  ...
  test('buildTaskCommitMessage: issueNumber appends Resolves trailer', () => {
    ...
    assert.ok(msg.includes("OTTO-Task: S01/T03"), "OTTO-Task trailer present");
    // OTTO-Task should come before Resolves
    const taskIdx = msg.indexOf("OTTO-Task: S01/T03");
    const resolvesIdx = msg.indexOf("Resolves #42");
    assert.ok(taskIdx < resolvesIdx, "OTTO-Task trailer before Resolves trailer");
  });

  {
    ...
    assert.ok(msg.includes("OTTO-Task: S01/T04"), "OTTO-Task trailer still present");
  }
  ...
  // ─── autoCommit: symlinked .otto/workflow does NOT stage milestone artifacts (#2247) ──

  test('autoCommit: symlinked .otto/workflow does NOT stage milestone artifacts (#2247)', () => {
    // When .otto/workflow is a symlink (external state project), .otto/workflow/ files live outside
    // the repo by design. smartStage() must NOT force-stage them into git — the
    // .gitignore exclusion is correct and intentional.
    const repo = initTempRepo();

    // Create an external .otto/workflow directory and symlink it into the repo
    ...
    assert.ok(!committed.includes(".otto/workflow/milestones/"),
      "symlink autoCommit: .otto/workflow/milestones/ files are NOT staged (external state stays external)");

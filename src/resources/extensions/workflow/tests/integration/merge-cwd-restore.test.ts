/**
 * the agent — Regression tests for merge cwd restore (#2929)
...
describe("merge cwd restore (#2929)", () => {
  ...
  test("autoCommitDirtyState does not commit on integration branch in worktree mode", () => {
    ...
    writeFileSync(
      join(repo, ".otto/workflow", "milestones", "M010", "M010-ROADMAP.md"),
    ...
    const squashCommit = run("git log --format=%H --grep='OTTO-Milestone: M010' -1", repo);

/**
 * state-machine-edge-cases.test.ts — Gap-filling tests for the OTTO state
...
/**
 * Create a standard .otto/workflow/ fixture with M001 containing S01 (2 tasks) and S02 (1 task).
...
describe("dispatch failure modes", () => {
  ...
  test("UAT verdict gate: roadmap-scoped ASSESSMENT verdict is ignored", async () => {
    ...
    const s01Dir = join(base, ".otto/workflow", "milestones", "M001", "slices", "S01");
    const assessmentRelPath = join(".otto/workflow", "milestones", "M001", "slices", "S01", "S01-ASSESSMENT.md");
    writeFileSync(
      join(s01Dir, "S01-ASSESSMENT.md"),
...
describe("ghost milestone edge cases", () => {
  ...
  test("empty directory with worktree is NOT a ghost (#2921)", () => {
    ...
    mkdirSync(join(base, ".otto/workflow", "worktrees", "M001"), { recursive: true });
...
describe("dispatch guard integration", () => {
  ...
  test("skip_milestone_validation preference writes pass-through VALIDATION", async () => {
    ...
    writeFileSync(
      join(base, ".otto/workflow", "milestones", "M001", "slices", "S02", "S02-SUMMARY.md"),
      "# S02 Summary\nDone.\n",
    );
  ...
  test("rewrite-docs circuit breaker: exceeding MAX attempts resolves all overrides", async () => {
    ...
    const runtimeDir = join(base, ".otto/workflow", "runtime");

...
test("runDispatch checks prior-slice completion against the project root in worktree mode", async () => {
  ...
  const ic = makeIC(deps, {
    s: {
      ...makeSession(),
      basePath: "/tmp/project/.otto/workflow/worktrees/M029-xoklo9",
...
test("runDispatch retries when complete-milestone summary exists on disk and stuck recovery can proceed (#4289)", async (t) => {
  ...
  execFileSync("git", ["add", ".otto/workflow/milestones/M001/M001-SUMMARY.md", "src/app.ts"], { cwd: base, stdio: "ignore" });

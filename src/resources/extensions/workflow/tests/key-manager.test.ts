...
test("formatKeyDashboard includes header and category sections", () => {
  ...
  assert.ok(output.includes("OTTO API Key Manager"));
...
// `/otto keys add alibaba-coding-plan` to silently fail (provider not found).
...
test("regression #3891 — alibaba-coding-plan is in PROVIDER_REGISTRY", () => {
  ...
  assert.ok(provider, "alibaba-coding-plan must be in PROVIDER_REGISTRY for /otto keys add to work");
...
test("alibaba-dashscope is in PROVIDER_REGISTRY", () => {
  ...
  assert.ok(provider, "alibaba-dashscope must be in PROVIDER_REGISTRY for /otto keys add to work");

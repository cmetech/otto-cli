...
test("shared/mod.ts imports without resolving @otto/pi-tui", () => {
  ...
  writeFileSync(
    ...
    [
      ...
      "  if (specifier === '@otto/pi-tui') throw new Error('unexpected @otto/pi-tui import');",
  ...
  try {
    ...
    assert.equal(
      result.status,
      0,
      `shared/mod.ts should import without @otto/pi-tui; stderr:\n${result.stderr}`,
    );
  } finally {

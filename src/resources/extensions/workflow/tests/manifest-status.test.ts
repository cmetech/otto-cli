 ...
 *
 * Uses temp directories with real .otto/workflow/milestones/M001/ structure.
 */
...
describe('getManifestStatus: mixed statuses', () => {
  ...
  beforeEach(() => {
    ...
    savedVal = process.env.OTTO_TEST_EXISTING_KEY_001;
    process.env.OTTO_TEST_EXISTING_KEY_001 = 'some-value';
  });
  afterEach(() => {
    delete process.env.OTTO_TEST_EXISTING_KEY_001;
    if (savedVal !== undefined) process.env.OTTO_TEST_EXISTING_KEY_001 = savedVal;
...
`);
    ...
    assert.deepStrictEqual(result!.existing, ['OTTO_TEST_EXISTING_KEY_001']);
  });
});
...
describe('getManifestStatus: key in env overrides manifest status', () => {
  ...
  beforeEach(() => {
    ...
    savedVal = process.env.OTTO_TEST_OVERRIDE_KEY;
    process.env.OTTO_TEST_OVERRIDE_KEY = 'already-here';
  });
  afterEach(() => {
    delete process.env.OTTO_TEST_OVERRIDE_KEY;
    if (savedVal !== undefined) process.env.OTTO_TEST_OVERRIDE_KEY = savedVal;
    rmSync(tmp, { recursive: true, force: true });
  });
...
`);
    ...
    assert.deepStrictEqual(result!.collected, []);
    assert.deepStrictEqual(result!.skipped, []);
    assert.deepStrictEqual(result!.existing, ['OTTO_TEST_OVERRIDE_KEY']);
  });
});

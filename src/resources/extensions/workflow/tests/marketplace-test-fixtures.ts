...
const CLONE_FIXTURES_ENABLED = process.env.OTTO_TEST_CLONE_MARKETPLACES === '1';
...
export function getMarketplaceFixtures(testFileDir: string): { available: boolean; skipReason?: string; fixtures?: MarketplaceFixtureSet } {
  ...
  if (!CLONE_FIXTURES_ENABLED) {
    return {
      available: false,
      skipReason: 'Marketplace repos absent and clone-based fixtures are disabled (set OTTO_TEST_CLONE_MARKETPLACES=1 to enable)',
    };
  }

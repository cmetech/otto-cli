...
test("ensureProjectWorkflowMcpConfig creates .mcp.json with the workflow server", () => {
  ...
  try {
    ...
    const server = parsed.mcpServers?.[OTTO_WORKFLOW_MCP_SERVER_NAME];
    ...
    assert.equal(server?.env?.OTTO_WORKFLOW_PROJECT_ROOT, projectRoot);
    assert.match(server?.env?.OTTO_WORKFLOW_EXECUTORS_MODULE ?? "", /workflow-tool-executors\.(js|ts)$/);
    assert.match(server?.env?.OTTO_WORKFLOW_WRITE_GATE_MODULE ?? "", /write-gate\.(js|ts)$/);
    if ((server?.env?.OTTO_WORKFLOW_EXECUTORS_MODULE ?? "").endsWith(".ts")) {
...
test("ensureProjectWorkflowMcpConfig preserves existing mcp servers", () => {
  ...
  try {
    ...
    assert.ok(parsed.mcpServers?.[OTTO_WORKFLOW_MCP_SERVER_NAME]);
  } finally {
    rmSync(projectRoot, { recursive: true, force: true });
  }
});

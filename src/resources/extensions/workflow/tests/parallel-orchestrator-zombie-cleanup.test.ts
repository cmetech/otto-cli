...
function makeTmpBase(): string {
  ...
  mkdirSync(join(base, ".otto/workflow", "parallel"), { recursive: true });
  return base;
}
...
function writePersistedState(basePath: string, data: PersistedState): void {
  const dest = join(basePath, ".otto/workflow", "orchestrator.json");
  writeFileSync(dest, JSON.stringify(data, null, 2), "utf-8");
}

/** Write a fake session status file to .otto/workflow/parallel/<milestoneId>.status.json */
function writeSessionStatusFile(
  ...
  const dest = join(basePath, ".otto/workflow", "parallel", `${milestoneId}.status.json`);
  writeFileSync(
    dest,
    JSON.stringify({
      milestoneId,
...
test("#2736: restoreRuntimeState clears stale state when all workers are stopped", (t) => {
  ...
  const stateFile = join(base, ".otto/workflow", "orchestrator.json");

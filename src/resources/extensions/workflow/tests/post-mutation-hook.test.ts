// OTTO Extension — post-mutation hook regression tests
...
test('post-mutation-hook: state-manifest.json exists after handleCompleteTask', async () => {
  ...
  try {
    ...
    const manifestPath = path.join(base, '.otto/workflow', 'state-manifest.json');
...
test('post-mutation-hook: handler still returns success even if projections dir is missing', async () => {
  // basePath with NO .otto/workflow directory — projections will fail to find milestones
  ...
  const tasksDir = path.join(base, '.otto/workflow', 'milestones', 'M001', 'slices', 'S01', 'tasks');
  fs.mkdirSync(tasksDir, { recursive: true });

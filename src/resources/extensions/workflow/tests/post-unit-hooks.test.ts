// OTTO Extension — Hook Engine Tests (Post-Unit, Pre-Dispatch, State Persistence)
...
test('resolveHookArtifactPath', () => {
  ...
  assert.deepStrictEqual(
    taskPath,
    join(base, ".otto/workflow", "milestones", "M001", "slices", "S01", "tasks", "T01-REVIEW-PASS.md"),
  ...
  assert.deepStrictEqual(
    slicePath,
    join(base, ".otto/workflow", "milestones", "M001", "slices", "S01", "REVIEW-PASS.md"),
  ...
  assert.deepStrictEqual(
    milestonePath,
    join(base, ".otto/workflow", "milestones", "M001", "REVIEW-PASS.md"),
...
test('State persistence: persist and restore', () => {
  ...
  try {
    ...
    const filePath = join(base, ".otto/workflow", "hook-state.json");
...
test('State persistence: restore from disk', () => {
  ...
  try {
    ...
    const stateFile = join(base, ".otto/workflow", "hook-state.json");
...
test('State persistence: restore handles corrupt file', () => {
  ...
  try {
    resetHookState();
    writeFileSync(join(base, ".otto/workflow", "hook-state.json"), "not json", "utf-8");
    // Should not throw
    restoreHookState(base);

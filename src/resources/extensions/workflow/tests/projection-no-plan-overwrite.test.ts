...
describe('renderAllProjections must not overwrite PLAN.md (#3651)', () => {
  it('preserves authoritative PLAN.md while rendering other projections', async () => {
    ...
    try {
      ...
      assert.ok(readFileSync(join(base, '.otto/workflow', 'STATE.md'), 'utf-8').includes('M001'))
      assert.ok(readFileSync(join(base, '.otto/workflow', 'ROADMAP.md'), 'utf-8').includes('M001'))
      assert.ok(readFileSync(join(base, '.otto/workflow', 'QUEUE.md'), 'utf-8').includes('M001'))
    } finally {
      closeDatabase()

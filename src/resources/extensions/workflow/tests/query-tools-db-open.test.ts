 ...
 *
 * otto_milestone_status previously called isDbAvailable() but never
...
describe('query-tools ensureDbOpen usage (#3672)', () => {
  test('otto_milestone_status opens the workspace DB before querying', async () => {
    ...
    try {
      ...
      process.chdir(base);
      const result = await tools.otto_milestone_status.execute(
        'call-1',
        { milestoneId: 'M001' },
        undefined,
        undefined,

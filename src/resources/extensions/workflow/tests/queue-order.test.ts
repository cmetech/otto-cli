...
test('test block at line 76', () => {
  ...
  const raw = JSON.parse(readFileSync(join(base, '.otto/workflow', 'QUEUE-ORDER.json'), 'utf-8'));
...
test('test block at line 90', () => {
  const base = createFixtureBase();
  writeFileSync(join(base, '.otto/workflow', 'QUEUE-ORDER.json'), 'not json');
...
test('test block at line 98', () => {
  const base = createFixtureBase();
  writeFileSync(join(base, '.otto/workflow', 'QUEUE-ORDER.json'), '{"order": "invalid"}');
...
test('test block at line 121', () => {
  ...
  assert.ok(!existsSync(join(base, '.otto/workflow', 'QUEUE-ORDER.json')), 'prune does not create file');
  cleanup(base);
});

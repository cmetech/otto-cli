...
describe("quick task turn_end cleanup (#2668)", () => {
  it("turn_end handler runs cleanupQuickBranch and removes quick-return state", async () => {
    ...
    try {
      ...
      writeFileSync(join(repo, ".otto/workflow", "runtime", "quick-return.json"), JSON.stringify({
        basePath: repo,
        originalBranch: "main",
      ...
      assert.equal(existsSync(join(repo, ".otto/workflow", "runtime", "quick-return.json")), false);
    } finally {

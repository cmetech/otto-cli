 ...
 * after cancelled /otto sessions.
 *
 * The module-level `reservedMilestoneIds` Set persists across /otto invocations
...
describe("stale milestone ID reservation cleanup (#2488)", () => {
  ...
  test("without cleanup, cancelled sessions inflate the next ID", () => {
    ...
    // Session 1: user starts /otto, ID is previewed and reserved, then cancelled
    ...
    // Session 2: user starts /otto again — stale reservation still in Set

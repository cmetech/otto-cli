...
test("buildContextMessage marks hidden guided context when memory is supplied", () => {
  const message = buildContextMessage({
    memoryBlock: "\n\n[MEMORY]\n\n- keep this",
    injection: "[OTTO Guided Execute Context]\nUse the task plan.",
  ...
  assert.match(message.content, /\[OTTO Context Metadata\]\n- Memory supplied: yes/);
  assert.ok(
    message.content.indexOf("Memory supplied: yes") < message.content.indexOf("[OTTO Guided Execute Context]"),
    "memory marker should appear before guided context",
  );
});

test("buildContextMessage caps hidden context by default", () => {
  const original = process.env.PI_OTTO_CONTEXT_MAX_CHARS;
  delete process.env.PI_OTTO_CONTEXT_MAX_CHARS;
  try {
    const message = buildContextMessage({
      memoryBlock: "",
      injection: `[OTTO Guided Execute Context]\n${"large context\n".repeat(500)}`,
    ...
    assert.match(message.content, /\[OTTO Context Truncated\]/);
  } finally {
    if (original === undefined) delete process.env.PI_OTTO_CONTEXT_MAX_CHARS;
    else process.env.PI_OTTO_CONTEXT_MAX_CHARS = original;
  }
});

test("buildContextMessage supports explicit context cap override", () => {
  const original = process.env.PI_OTTO_CONTEXT_MAX_CHARS;
  process.env.PI_OTTO_CONTEXT_MAX_CHARS = "1200";
  try {
    const message = buildContextMessage({
      memoryBlock: "",
      injection: `[OTTO Guided Execute Context]\n${"large context\n".repeat(200)}`,
...
test("isLowEntropyResumePrompt identifies bare resume prompts only", () => {
  ...
  assert.equal(isLowEntropyResumePrompt("/otto auto"), false);
});

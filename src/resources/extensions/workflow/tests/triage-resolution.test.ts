/**
 * Unit tests for OTTO Triage Resolution — resolution execution and file overlap detection.
 */
...
// Import only the functions that don't depend on @otto/pi-coding-agent
...
function setupPlanFile(tmp: string, mid: string, sid: string, content: string): string {
  const planDir = join(tmp, ".otto/workflow", "milestones", mid, "slices", sid);
...
test("resolution: executeReplan writes REPLAN-TRIGGER.md", () => {
  ...
  try {
    ...
    const triggerPath = join(
      tmp, ".otto/workflow", "milestones", "M001", "slices", "S01", "S01-REPLAN-TRIGGER.md",
    );
...
test("resolution: markCaptureExecuted is idempotent", () => {
  ...
  try {
    ...
    const filePath = join(tmp, ".otto/workflow", "CAPTURES.md");
...
test("resolution: executeTriageResolutions executes inject captures", () => {
  ...
  try {
    ...
    const planPath = join(tmp, ".otto/workflow", "milestones", "M001", "slices", "S01", "S01-PLAN.md");
...
test("resolution: ensureDeferMilestoneDir creates milestone directory with CONTEXT-DRAFT.md", () => {
  ...
  try {
    mkdirSync(join(tmp, ".otto/workflow", "milestones"), { recursive: true });
    ...
    const msDir = join(tmp, ".otto/workflow", "milestones", "M005");
...
test("resolution: ensureDeferMilestoneDir returns true without overwriting existing directory", () => {
  ...
  try {
    const msDir = join(tmp, ".otto/workflow", "milestones", "M003");
...
test("resolution: ensureDeferMilestoneDir handles unique milestone IDs (M005-abc123)", () => {
  ...
  try {
    ...
    const msDir = join(tmp, ".otto/workflow", "milestones", "M005-abc123");
    assert.ok(existsSync(msDir), "milestone directory should exist");
    assert.ok(
...
test("resolution: executeTriageResolutions creates milestone dir for deferred captures", () => {
  ...
  try {
    ...
    assert.ok(
      existsSync(join(tmp, ".otto/workflow", "milestones", "M005")),
      "M005 directory should exist",
    );
    assert.ok(
      existsSync(join(tmp, ".otto/workflow", "milestones", "M005", "M005-CONTEXT-DRAFT.md")),
    ...
    const draft = readFileSync(join(tmp, ".otto/workflow", "milestones", "M005", "M005-CONTEXT-DRAFT.md"), "utf-8");

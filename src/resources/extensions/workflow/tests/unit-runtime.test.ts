...
const tasksDir = join(base, ".otto/workflow", "milestones", "M100", "slices", "S02", "tasks");
mkdirSync(tasksDir, { recursive: true });
writeFileSync(join(base, ".otto/workflow", "STATE.md"), "## Next Action\nExecute T09 for S02: do the thing\n", "utf-8");
writeFileSync(
  join(base, ".otto/workflow", "milestones", "M100", "slices", "S02", "S02-PLAN.md"),
...
console.log("\n=== execute-task durability trusts closed DB task status ===");
{
  ...
  mkdirSync(join(dbBase, ".otto/workflow", "milestones", "M300", "slices", "S01", "tasks"), { recursive: true });
  try {
    openDatabase(join(dbBase, ".otto/workflow", "otto.db"));
    ...
    writeFileSync(
      join(dbBase, ".otto/workflow", "milestones", "M300", "slices", "S01", "S01-PLAN.md"),
      "# S01\n\n## Tasks\n\n- [ ] **T01: DB Task** `est:10m`\n",
      "utf-8",
    );
    writeFileSync(join(dbBase, ".otto/workflow", "STATE.md"), "## Next Action\nExecute T01 for S01: DB task\n", "utf-8");
...
console.log("\n=== hook unit type sanitization (slash in unitType) ===");
{
  ...
  const unitsDir = join(base, ".otto/workflow", "runtime", "units");
...
{
  const tasksDir2 = join(mhBase, ".otto/workflow", "milestones", "M200", "slices", "S01", "tasks");
  mkdirSync(tasksDir2, { recursive: true });

  // Slice plan with T01 checked
  writeFileSync(
    join(mhBase, ".otto/workflow", "milestones", "M200", "slices", "S01", "S01-PLAN.md"),
  ...
  writeFileSync(join(mhBase, ".otto/workflow", "STATE.md"), "## Next Action\nExecute T02 for S01: next thing\n", "utf-8");
...
{
  const tasksDir3 = join(mhBase, ".otto/workflow", "milestones", "M200", "slices", "S02", "tasks");
  mkdirSync(tasksDir3, { recursive: true });

  writeFileSync(
    join(mhBase, ".otto/workflow", "milestones", "M200", "slices", "S02", "S02-PLAN.md"),
...
{
  const tasksDir4 = join(mhBase, ".otto/workflow", "milestones", "M200", "slices", "S03", "tasks");
  mkdirSync(tasksDir4, { recursive: true });

  writeFileSync(
    join(mhBase, ".otto/workflow", "milestones", "M200", "slices", "S03", "S03-PLAN.md"),
...
{
  const tasksDir5 = join(mhBase, ".otto/workflow", "milestones", "M200", "slices", "S04", "tasks");
  mkdirSync(tasksDir5, { recursive: true });

  writeFileSync(
    join(mhBase, ".otto/workflow", "milestones", "M200", "slices", "S04", "S04-PLAN.md"),
...
{
  const tasksDir6 = join(mhBase, ".otto/workflow", "milestones", "M200", "slices", "S05", "tasks");
  mkdirSync(tasksDir6, { recursive: true });

  writeFileSync(
    join(mhBase, ".otto/workflow", "milestones", "M200", "slices", "S05", "S05-PLAN.md"),
...
console.log("\n=== per-record lock: stale .lock is reclaimed, list ignores .lock files ===");
{
  ...
  try {
    const unitsDir = join(lockBase, ".otto/workflow", "runtime", "units");
  ...
  try {
    ...
    const unitsDir = join(listBase, ".otto/workflow", "runtime", "units");

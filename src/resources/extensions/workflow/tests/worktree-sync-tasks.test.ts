 ...
 * worktree .otto/workflow directory are legacy projections. syncWorktreeStateBack must
 * not copy them into the canonical project .otto/workflow tree.
 */
...
test("syncWorktreeStateBack does not copy task markdown projections from worktree", () => {
  ...
  try {
    writeFile(wtBase, ".otto/workflow/milestones/M001/M001-ROADMAP.md", "# Roadmap\n");
    writeFile(wtBase, ".otto/workflow/milestones/M001/slices/S01/S01-PLAN.md", "# Plan\n");
    writeFile(wtBase, ".otto/workflow/milestones/M001/slices/S01/tasks/T01-SUMMARY.md", "# Task Summary\n");
    ...
    assert.equal(
      existsSync(join(mainBase, ".otto/workflow/milestones/M001/M001-ROADMAP.md")),
      false,
      "milestone markdown projection must not be copied",
    );
    assert.equal(
      existsSync(join(mainBase, ".otto/workflow/milestones/M001/slices/S01/tasks/T01-SUMMARY.md")),
      false,
...
test("syncWorktreeStateBack still copies diagnostic root files", () => {
  ...
  try {
    writeFile(wtBase, ".otto/workflow/completed-units.json", JSON.stringify({ units: ["M001/S01/T01"] }));
    writeFile(wtBase, ".otto/workflow/metrics.json", JSON.stringify({ version: 1, units: [] }));
    mkdirSync(join(mainBase, ".otto/workflow"), { recursive: true });
    ...
    assert.ok(existsSync(join(mainBase, ".otto/workflow/completed-units.json")));
    assert.ok(existsSync(join(mainBase, ".otto/workflow/metrics.json")));
  } finally {
    cleanup(mainBase, wtBase);
  }
});

...
writeFileSync(join(base, ".otto/workflow", "milestones", "M001", "M001-ROADMAP.md"), `# M001: Demo\n\n## Slices\n- [ ] **S01: Slice One** \`risk:low\` \`depends:[]\`\n  > After this: demo works\n`, "utf-8");
writeFileSync(join(base, ".otto/workflow", "milestones", "M001", "slices", "S01", "S01-PLAN.md"), `# S01: Slice One\n\n**Goal:** Demo\n**Demo:** Demo\n\n## Must-Haves\n- done\n\n## Tasks\n- [ ] **T01: Implement** \`est:10m\`\n  do it\n`, "utf-8");
run("git add .", base);
...
describe('worktree', async () => {
  ...
  assert.deepStrictEqual(detectWorktreeName("/projects/myapp/.otto/workflow/worktrees/feature-auth"), "feature-auth", "detects worktree name");
  assert.deepStrictEqual(detectWorktreeName("/projects/myapp/.otto/workflow/worktrees/my-wt/subdir"), "my-wt", "detects worktree with subdir");
...

  {
    ...
    const fakeHome = mkdtempSync(join(tmpdir(), "otto-fake-home-"));
  ...
  assert.deepStrictEqual(
    detectWorktreeName("/Users/fran/.otto/workflow/projects/89e1c9ad49bf/worktrees/M001"),
    "M001",
    "detects milestone in symlink-resolved path",
  );
  assert.deepStrictEqual(
    detectWorktreeName("/Users/fran/.otto/workflow/projects/abc123/worktrees/M002/subdir"),
    "M002",
    "detects milestone with trailing subdir in symlink-resolved path",
  );
  assert.deepStrictEqual(
    detectWorktreeName("/Users/fran/.otto/workflow/projects/abc123"),
    null,
    "returns null for project root without worktrees segment",
  );
  assert.deepStrictEqual(
    detectWorktreeName("/foo/.otto/workflow/worktrees/M001"),
    "M001",
  ...
  // BUG FIX: symlink-resolved paths that land inside ~/.otto should NOT
  ...
  // With OTTO_PROJECT_ROOT env var set (layer 1 — coordinator passes it)
  process.env.OTTO_PROJECT_ROOT = "/real/project";
  assert.deepStrictEqual(
    resolveProjectRoot("/Users/fran/.otto/workflow/projects/89e1c9ad49bf/worktrees/M001"),
    "/real/project",
    "uses OTTO_PROJECT_ROOT when set",
  );
  delete process.env.OTTO_PROJECT_ROOT;

  // Without OTTO_PROJECT_ROOT, direct layout still works (no ~/.otto collision)
  assert.deepStrictEqual(
    resolveProjectRoot("/some/repo"),
    "/some/repo",
    "ignores OTTO_PROJECT_ROOT override for non-worktree paths",
  );
  delete process.env.OTTO_PROJECT_ROOT;

  // Without OTTO_PROJECT_ROOT, direct layout still works (no ~/.otto collision)
  assert.deepStrictEqual(
    resolveProjectRoot("/foo/.otto/workflow/worktrees/M001"),
    "/foo",
  ...
  // Without OTTO_PROJECT_ROOT, direct layout with nested subdirs
  assert.deepStrictEqual(
    resolveProjectRoot("/data/.otto/workflow/worktrees/M003/nested"),
    "/data",
  ...
  // Real symlink + git worktree scenario, with deep nested path from cwd
  {
    ...
    mkdirSync(join(project, ".otto"), { recursive: true });
    symlinkSync(storage, join(project, ".otto/workflow"));
    ...
    run("git worktree add .otto/workflow/worktrees/M001 -b worktree/M001", project);

    const deep = join(project, ".otto/workflow", "worktrees", "M001", "a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k");

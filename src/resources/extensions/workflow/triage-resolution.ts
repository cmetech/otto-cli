...
import { workflowRoot, milestonesDir } from "./paths.js";
...
export function executeInject(
  ...
  try {
    ...
    const planPath = join(workflowRoot(basePath), "milestones", mid, "slices", sid, `${sid}-PLAN.md`);
...
export function executeReplan(
  ...
  try {
    const triggerPath = join(
      basePath, ".otto/workflow", "milestones", mid, "slices", sid, `${sid}-REPLAN-TRIGGER.md`,
    );
 ...
 *
 * Writes a BACKTRACK-TRIGGER.md marker at `.otto/workflow/BACKTRACK-TRIGGER.md` with
...
export function executeBacktrack(
  ...
  try {
    ...
    const ts = new Date().toISOString();
    const triggerPath = join(workflowRoot(basePath), "BACKTRACK-TRIGGER.md");
    const content = [
      `# Backtrack Trigger`,
      ``,
      `**Source:** Capture ${capture.id}`,
...
export function buildQuickTaskPrompt(capture: CaptureEntry): string {
  return [
    `You are executing a quick one-off task captured during a auto-mode session.`,
    ``,
    `## Quick Task`,
    ``,
    ...
    `3. Do NOT modify any \`.otto/workflow/\` plan files — this is a one-off, not a planned task.`,

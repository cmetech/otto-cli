...
function envForcesLegacyFallback(): boolean {
  const raw = (process.env.LOOP24_UOK_FORCE_LEGACY ?? process.env.GSD_UOK_FORCE_LEGACY) ?? (process.env.LOOP24_UOK_LEGACY_FALLBACK ?? process.env.GSD_UOK_LEGACY_FALLBACK);
  if (!raw) return false;

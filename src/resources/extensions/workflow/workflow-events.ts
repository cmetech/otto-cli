...
/**
 * Append one event to .otto/workflow/event-log.jsonl.
 * Computes a content hash from cmd+params (deterministic, independent of ts/actor/session).
 * Creates .otto/workflow directory if needed.
 */
 ...
 *
 * @param basePath - Project root (parent of .otto/workflow/)
...
export function compactMilestoneEvents(
  ...
  const logPath = join(basePath, ".otto/workflow", "event-log.jsonl");
  const archivePath = join(basePath, ".otto/workflow", `event-log-${milestoneId}.jsonl.archived`);

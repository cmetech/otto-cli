// Workflow Extension — Legacy Markdown to Engine Migration
...
/**
 * Returns true when engine tables are empty AND a .otto/workflow/milestones/ directory
...
export function needsAutoMigration(basePath: string): boolean {
  ...
  // Check if .otto/workflow/milestones/ directory exists
  const milestonesDir = join(basePath, ".otto/workflow", "milestones");
...
/**
 * Migrate legacy markdown-only .otto/workflow/ projects to engine DB state.
 * Reads .otto/workflow/milestones/<ID>/ directories and parses ROADMAP.md, *-PLAN.md
...
export function migrateFromMarkdown(basePath: string): void {
  ...
  if (!existsSync(milestonesDir)) {
    process.stderr.write("workflow-migration: no .otto/workflow/milestones/ directory found, nothing to migrate\n");
    return;
  }
  ...
  if (milestoneDirs.length === 0) {
    process.stderr.write("workflow-migration: no milestone directories found in .otto/workflow/milestones/\n");
    return;
  }

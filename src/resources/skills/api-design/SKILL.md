...
<context>
the agent has `design-an-interface` for general module-interface design; this skill is the HTTP/GraphQL specialization. REST and GraphQL carry baggage — status codes, verbs, nullability, pagination — that a generic interface-design discussion glosses over.
...
Append architectural decisions to `.otto/workflow/DECISIONS.md`.
...
- [ ] Decisions appear in `.otto/workflow/DECISIONS.md`.

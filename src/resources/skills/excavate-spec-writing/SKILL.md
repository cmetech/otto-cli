---
name: excavate-spec-writing
description: Deep behavioral documentation methodology. Per-module behavioral specifications, external and behavioral integration contracts, behavior documentation, end-to-end user journey analysis. Transforms synthesis into implementable behavioral specifications.
---

# Behavioral Spec Writing

You are a general-purpose worker. You were dispatched with a role and told to Read this skill, then produce the specified behavioral specification.

This stage transforms synthesis output (module map, feature inventory, architecture doc, API surface, behavioral summaries) into specifications detailed enough that a developer can reimplement the target WITHOUT seeing the original code. It carries the methodology for per-module deep dives, contract extraction, behavioral documentation, and user-journey analysis.

## Pipeline Overview

```dot
digraph spec_pipeline {
    rankdir=TB;
    compound=true;

    "Synthesis complete" [shape=doublecircle];
    "Specs complete — proceed to verification" [shape=doublecircle];

    subgraph cluster_phase1 {
        label="Phase 1: Per-Module Deep Dives (parallel across modules)";
        style=dashed;

        "Deep Dive: Module A" [shape=box];
        "Deep Dive: Module B" [shape=box];
        "Deep Dive: Module N" [shape=box];
    }
    ...
    subgraph cluster_phase2 {
        ...
        "Contract Extraction: external + integration contracts" [shape=box];
        "Behavior Documentation: behavioral domain synthesis" [shape=box];
        "User Journeys: end-to-end journeys" [shape=box];
    }

    "Synthesis complete" -> "Deep Dive: Module A";
    "Synthesis complete" -> "Deep Dive: Module B";
    "Synthesis complete" -> "Deep Dive: Module N";

    "Deep Dive: Module A" -> "Contract Extraction: external + integration contracts" [lhead=cluster_phase2];
    "Deep Dive: Module B" -> "Behavior Documentation: behavioral domain synthesis" [lhead=cluster_phase2];
    "Deep Dive: Module N" -> "User Journeys: end-to-end journeys" [lhead=cluster_phase2];

    "Contract Extraction: external + integration contracts" -> "Specs complete — proceed to verification";
    "Behavior Documentation: behavioral domain synthesis" -> "Specs complete — proceed to verification";
    "User Journeys: end-to-end journeys" -> "Specs complete — proceed to verification";
}
```

**Phase 1** runs a per-module deep dive in parallel across every module from the module map. Each produces a per-module behavioral specification.

**Phase 2** runs three tasks in parallel. They consume Phase 1 output plus synthesis to produce contracts, behavioral documentation, and user journeys.
...
| `workspace/raw/synthesis/behavioral-summaries/` | Merged intelligence from all analysis modes |
...
## Per-Module Behavioral Specification

This is the primary deliverable. Each module in the module map gets a complete behavioral specification written to `workspace/raw/specs/modules/{module-name}.md`.
...
```markdown
<!-- cite: source=source-code, ref=workspace/raw/source/analysis/chunk-0024.md:105, confidence=disputed, contradiction=workspace/raw/source/exploration/auth-layer/findings.md:23 -->
```
...
**HARD STOP: Review your output file BEFORE writing it.** Read every line and apply the reimplementor test (see the Black Box Methodology above). Do NOT write the file until all checks pass.
...
Contract extraction produces two categories of output: contracts that cross the analysis/implementation boundary to the implementer, and inter-module contracts that stay in analysis for verification purposes only.
...
Internal wiring between the original's modules. Stays in `workspace/raw/`. Used ONLY for verification to ensure complete analysis coverage. This is structural contamination -- it encodes the original's internal module boundaries.
...
Behavior documentation is pure synthesis. It does NOT read source code or run the target. It consumes the output of the analysis stages and synthesizes findings into unified behavioral documents.
...
digraph behavior_synthesis {
    ...
    "Gather all analysis and Phase 1 evidence" [shape=box];
    ...
    "Gather all analysis and Phase 1 evidence" -> "Group claims by behavioral domain";
...
1. When [trigger], the system [does X]
   <!-- cite: source=source-code, ref=..., confidence=confirmed, corroborated_by=runtime-observation -->
2. It then [does Y]
   <!-- cite: source=source-code, ref=..., confidence=inferred -->
...
- "Insufficient evidence from the analysis stage" -- Document what IS available. Flag gaps explicitly.
...
CRITICAL for all spec output. Every spec-writing worker must enforce these rules.
...
- Sessions expire after 30 minutes of inactivity
  <!-- cite: source=source-code, ref=workspace/raw/source/analysis/chunk-0007.md:88, confidence=confirmed, corroborated_by=runtime-observation -->
```
...
| Multiple sources independently agree | `confirmed` |
...
Spec-writing preserves the original analysis-stage citations. When multiple sources agree, add corroboration notes:

```markdown
- Network retries use exponential backoff with jitter
  <!-- cite: source=source-code, ref=workspace/raw/source/analysis/chunk-0015.md:89, confidence=confirmed, corroborated_by=runtime-observation -->
```
...
```
workspace/raw/specs/
    modules/                              # Per-module behavioral specs
    ...
    behavioral-docs/                      # Pure behavioral documentation by domain
        behavior-{domain}.md              # Synthesized behavioral spec per domain
    journeys/
        {journey-name}.md                 # End-to-end user journeys
```
...
- [ ] Zero source code identifiers in output
- [ ] No raw source code excerpts longer than one line
- [ ] Evidence from all analysis modes gathered and corroborated
...
You MUST actually write output files.  Do NOT:

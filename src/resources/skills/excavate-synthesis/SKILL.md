---
name: excavate-synthesis
description: Synthesis methodology. Feature discovery, architecture reverse engineering, API extraction, cross-source synthesis with conflict resolution, module mapping. Transforms raw analysis findings into structured synthesis documents.
---

# Multi-Source Synthesis

You are a general-purpose worker. You were dispatched with a role and told to Read this skill. Transform raw analysis findings into structured synthesis documents: merge findings from multiple independent analysis modes into corroborated, conflict-resolved behavioral descriptions that the spec-writing stage can write deep specs from.
...
- **Cite:** Every synthesized claim traces back to its source evidence with `<!-- cite: -->` annotations.
...
digraph synthesis_pipeline {
    ...
    subgraph cluster_phase1 {
        ...
        style=filled;
        fillcolor="#e8f4e8";
        "Feature Discovery" [shape=box];
        "Architecture Analysis" [shape=box];
        "API Extraction" [shape=box];
    }
    ...
    subgraph cluster_phase3 {
        ...
        style=filled;
        fillcolor="#f4e8e8";
        "Module Mapping" [shape=box];
    }

    "Source Code Analysis" -> "Feature Discovery";
    "Source Code Analysis" -> "Architecture Analysis";
    "Source Code Analysis" -> "API Extraction";
    "Public Documentation" -> "Feature Discovery";
    "Public Documentation" -> "Architecture Analysis";
    "Public Documentation" -> "API Extraction";

    "Feature Discovery" -> "Synthesis";
    "Architecture Analysis" -> "Synthesis";
    "API Extraction" -> "Synthesis";

    "Synthesis" -> "Module Mapping";
    "Feature Discovery" -> "Module Mapping";
    "Architecture Analysis" -> "Module Mapping";
    "API Extraction" -> "Module Mapping";

    "Module Mapping" -> "Spec Writing" [shape=doublecircle];
}
```

### Phase Execution Rules

- **Phase 1** tasks (feature discovery, architecture analysis, API extraction) can run in parallel. They share no output with each other. Each reads only the raw analysis directories.
- **Phase 2** runs after ALL Phase 1 tasks complete. Synthesis reads Phase 1 output AND the raw analysis findings.
- **Phase 3** runs after Phase 2 completes. Module mapping reads ALL Phase 1 and Phase 2 output.

No phase advances until the previous phase is fully complete.

## Input Sources

Synthesis consumes output from the analysis stage. Not all modes will be present for every run.
...
Phase 1 output (consumed by Phase 2 and Phase 3):
  workspace/raw/synthesis/features/       feature discovery output
  workspace/raw/synthesis/architecture/   architecture analysis output
  workspace/raw/synthesis/api/            API extraction output

Phase 2 output (consumed by Phase 3):
  workspace/raw/synthesis/behavioral-summaries/   synthesis output
...
Run this check before starting. Record results in output metadata.
...
At least one must be true. If no source contributed, halt with an error.
...
**Task:** Feature Discovery | **Phase:** 1 (parallel) | **Output:** `workspace/raw/synthesis/features/`
...
**Task:** Architecture Analysis | **Phase:** 1 (parallel) | **Output:** `workspace/raw/synthesis/architecture/`
...
**Task:** API Extraction | **Phase:** 1 (parallel) | **Output:** `workspace/raw/synthesis/api/`
...
**Task:** Synthesis | **Phase:** 2 | **Output:** `workspace/raw/synthesis/`

THIS IS THE CORE OF SYNTHESIS.

### Goal

Read ALL raw analysis and Phase 1 output. For each behavioral domain, collect claims from every available mode, identify agreements, contradictions, and gaps. Produce merged behavioral summaries with multi-source provenance. Produce the reimplementation essentials document.
...
digraph synthesis_process {
    ...
    "Read all Phase 1 + raw analysis output" [shape=doublecircle];
    ...
    "Read all Phase 1 + raw analysis output" -> "Identify behavioral domains";
...
- Public docs: "Sessions are invalidated after 30 minutes of inactivity"
  <!-- cite: source=official-docs, ref=workspace/public/docs/claims/claims-by-topic.md:89, confidence=confirmed -->
- Source code: SESSION_TIMEOUT constant = 1800000 (30 minutes in ms)
  <!-- cite: source=source-code, ref=workspace/raw/source/analysis/chunk-0058.md:91, confidence=inferred -->
- Runtime: Session expired after 30m idle in CLI test
  <!-- cite: source=runtime-observation, ref=workspace/raw/runtime/cli/session-test.md:34, confidence=confirmed -->

**Synthesis:** CONFIRMED by 3 independent sources.
<!-- cite: source=official-docs, ref=workspace/public/docs/claims/claims-by-topic.md:89, confidence=confirmed, corroborated_by=source-code,runtime-observation -->
```
...
## Metadata
- **Synthesized at:** synthesis stage
- **Date:** [date]
...
**Task:** Module Mapping | **Phase:** 3 | **Output:** `workspace/raw/synthesis/module-map.md`

### Scope Note

**The module map is an analysis organizational artifact.** It ensures every part of the target gets a deep behavioral spec. It belongs to the analysis workspace and does not appear in the output. The implementer is free to choose its own module boundaries, architecture, and internal decomposition.

Downstream packaging uses the module map to verify that all behavioral content has been accounted for, then merges module-organized specs into behavioral domain specs for the `output/` set.
...
- **Documentation readiness:** ready/needs-work for spec-writing deep dives
...
NOTE: ALL modules will receive a deep behavioral spec. Priority determines the implementer's implementation order, NOT analysis scope.
```
...
| Scenario | Handling |
|----------|---------|
| Source-only module (e.g., internal cache) | Include in map. Note "source-only". spec-writing relies on source analysis. |
| Runtime-only module (e.g., UI animations) | Include in map. Note "runtime-only". spec-writing relies on runtime observations. |
...
| Multi-mode module (e.g., authentication) | Include with full multi-source evidence. Highest confidence for spec-writing. |
...
After mapping all modules, produce a recommended order for spec-writing deep dives. Order by:
...
Synthesis tasks cite upstream artifacts, not raw source code. The citation chain is:

```
analysis stage → raw evidence → analysis output file (with citation)
synthesis stage → analysis output file → synthesis output file (with citation referencing the analysis file)
```

### Source Type Mapping

Synthesis preserves the original source type in citations:
...
Do NOT batch citations at the end. Every time you write a behavioral claim, the very next thing you write is the citation. This rule applies without exception.
...
- Every behavioral claim traces back to upstream analysis citations
...
- [ ] Cross-references to upstream analysis artifacts are valid file paths

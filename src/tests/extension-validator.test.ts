...
describe('checkDependencyPlacement', () => {
  test('returns error for @otto/ package in dependencies', () => {
    const errors = checkDependencyPlacement({ dependencies: { '@otto/pi-coding-agent': '^2.0.0' } })
    assert.equal(errors.length, 1)
    assert.equal(errors[0].code, 'WRONG_DEP_FIELD')
    assert.ok(errors[0].message.includes('@otto/pi-coding-agent'), 'message must name exact package')
  ...
  test('returns error for @otto/ package in devDependencies', () => {
    const errors = checkDependencyPlacement({ devDependencies: { '@otto/pi-ai': '^1.0.0' } })
    assert.equal(errors.length, 1)
    assert.equal(errors[0].code, 'WRONG_DEP_FIELD')
    assert.ok(errors[0].message.includes('@otto/pi-ai'), 'message must name exact package')
  ...
  test('does not flag @otto/ in peerDependencies', () => {
    const errors = checkDependencyPlacement({ peerDependencies: { '@otto/pi-coding-agent': '>=2.50.0' } })
  ...
  test('returns multiple errors for violations in both dependencies and devDependencies', () => {
    const errors = checkDependencyPlacement({
      ...
      devDependencies: { '@otto/pi-ai': '^1.0.0' },
    })
  ...
  test('returns empty errors when dependencies is a string instead of object', () => {
    const errors = checkDependencyPlacement({ dependencies: '@otto/pi-coding-agent' })
...
describe('edge cases — field types', () => {
  test('does not flag @otto/ package nested in sub-object of dependencies (only top-level keys matter)', () => {
    // The checker iterates Object.keys(deps) — a sub-object value is a value, not a key name
    const errors = checkDependencyPlacement({
      dependencies: { nested: { '@otto/foo': '1.0' } },
    })
    assert.equal(errors.length, 0, 'nested @otto/ in a sub-object value should not be flagged')
  })
})

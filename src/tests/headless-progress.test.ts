...
describe('summarizeToolArgs', () => {
  ...
  it('summarizes gsd tool with milestone/slice/task IDs', () => {
    assert.equal(summarizeToolArgs('otto_task_complete', {
      milestoneId: 'M001', sliceId: 'S01', taskId: 'T01', oneLiner: 'Built the thing',
    }), 'M001/S01/T01 Built the thing')
  })

  it('summarizes otto_plan_milestone with milestone ID', () => {
    assert.equal(summarizeToolArgs('otto_plan_milestone', { milestoneId: 'M002' }), 'M002')
  })

  it('summarizes otto_decision_save with decision text', () => {
    const result = summarizeToolArgs('otto_decision_save', { decision: 'Use SQLite for persistence' })

...
test("resolveSubprocessModule returns source .ts path when NOT under node_modules", () => {
  ...
  const result = resolveSubprocessModule(
    packageRoot,
    "resources/extensions/workflow/workspace-index.ts",
    // existsSync not needed — should return src path without checking dist
  )

  assert.deepEqual(result, {
    modulePath: join(packageRoot, "src", "resources/extensions/workflow/workspace-index.ts"),
    useCompiledJs: false,
  })
})
...
test("resolveSubprocessModule returns compiled .js path when under node_modules and dist file exists", () => {
  ...
  const distPath = join(packageRoot, "dist", "resources/extensions/workflow/workspace-index.js")
  const result = resolveSubprocessModule(
    packageRoot,
    "resources/extensions/workflow/workspace-index.ts",
    (p: string) => p === distPath,
...
test("resolveSubprocessModule handles Windows paths under node_modules", () => {
  ...
  const distPath = join(packageRoot, "dist", "resources/extensions/workflow/auto.js")
  const result = resolveSubprocessModule(
    packageRoot,
    "resources/extensions/workflow/auto.ts",
    (p: string) => p === distPath,
  )
...
test("resolveSubprocessModule strips .ts extension when building dist .js path", () => {
  ...
  resolveSubprocessModule(
    packageRoot,
    "resources/extensions/workflow/doctor.ts",
    (p: string) => { checkedPath = p; return true },
  )

  assert.equal(
    checkedPath,
    join(packageRoot, "dist", "resources/extensions/workflow/doctor.js"),
...
test("buildSubprocessPrefixArgs omits TS loaders when compiled JS was selected", () => {
  assert.deepEqual(
    buildSubprocessPrefixArgs(
      ...
      {
        modulePath: "/usr/lib/node_modules/@opengsd/gsd-pi/dist/resources/extensions/workflow/workspace-index.js",
        useCompiledJs: true,
      },
      "file:///loader.mjs",
    ),
...
test("buildSubprocessPrefixArgs keeps TS loader path when source TS was selected", () => {
  const args = buildSubprocessPrefixArgs(
    ...
    {
      modulePath: "/home/user/projects/gsd/src/resources/extensions/workflow/workspace-index.ts",
      useCompiledJs: false,
    },
    "file:///loader.mjs",
  )

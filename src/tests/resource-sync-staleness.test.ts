 ...
 * with a broken import to persist at ~/.otto/workflow/agent/extensions/).
 */
...
test("resource manifest includes contentHash", async (t) => {
  ...
  assert.equal(read.workflowVersion, "2.28.0");
...
test("missing contentHash in manifest triggers re-sync (upgrade path)", () => {
  ...
  const shouldSkip = oldManifest.workflowVersion === "2.28.0"
...
test("matching contentHash skips re-sync", () => {
  ...
  const shouldSkip = manifest.workflowVersion === "2.28.0"

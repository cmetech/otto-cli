...
export function stopWebMode(deps: Pick<WebModeDeps, 'pidFilePath' | 'readPidFile' | 'deletePidFile' | 'stderr'> = {}, options: WebModeStopOptions = {}): WebModeStopResult {
  ...
  if (options.all) {
    ...
    for (const [cwd, entry] of entries) {
      ...
      if (result === 'killed') {
        stderr.write(`[otto] Stopped web server for ${cwd} (pid=${entry.pid})\n`)
        stopped++
      } else if (result === 'already-dead') {
        stderr.write(`[otto] Web server for ${cwd} was already stopped (pid=${entry.pid})\n`)
        stopped++
      } else {
        stderr.write(`[otto] Failed to stop web server for ${cwd}: ${result.error}\n`)
      }
    ...
    stderr.write(`[otto] Stopped ${stopped} instance${stopped === 1 ? '' : 's'}.\n`)
  ...
  if (options.projectCwd) {
    ...
    if (!entry) {
      stderr.write(`[otto] No web server running for ${resolvedCwd}\n`)
      return { ok: false, reason: 'not-found' }
    }
    const result = killPid(entry.pid)
    unregisterInstance(resolvedCwd)
    if (result === 'killed') {
      stderr.write(`[otto] Stopped web server for ${resolvedCwd} (pid=${entry.pid})\n`)
      return { ok: true, stoppedCount: 1 }
    } else if (result === 'already-dead') {
      stderr.write(`[otto] Web server for ${resolvedCwd} was already stopped — cleared stale entry.\n`)
      return { ok: true, stoppedCount: 1 }
    } else {
      stderr.write(`[otto] Failed to stop web server for ${resolvedCwd}: ${result.error}\n`)
...
function stopLegacyPidFile(deps: Pick<WebModeDeps, 'pidFilePath' | 'readPidFile' | 'deletePidFile' | 'stderr'>): WebModeStopResult {
  ...
  if (pid === null) {
    stderr.write(`[otto] Web server is not running (no PID file found)\n`)
    return { ok: false, reason: 'no-pid-file' }
  }

  stderr.write(`[otto] Stopping web server (pid=${pid})…\n`)

  const result = killPid(pid)
  deletePid(pidFilePath)
  if (result === 'killed') {
    stderr.write(`[otto] Web server stopped.\n`)
    return { ok: true }
  } else if (result === 'already-dead') {
    stderr.write(`[otto] Web server was already stopped — cleared stale PID file.\n`)
    return { ok: true }
  } else {
    stderr.write(`[otto] Failed to stop web server: ${result.error}\n`)
    return { ok: false, reason: result.error }
  }
}
...
function formatLaunchStatus(status: WebModeLaunchStatus): string {
  if (status.ok) {
    return `[otto] Web mode startup: status=started cwd=${status.cwd} port=${status.port} host=${status.hostPath} kind=${status.hostKind} url=${status.url}\n`
  }

  return `[otto] Web mode startup: status=failed cwd=${status.cwd} port=${status.port ?? 'n/a'} host=${status.hostPath ?? 'unresolved'} kind=${status.hostKind} reason=${status.failureReason}\n`
}
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        if (!hostUp) {
          hostUp = true
          stderr?.write(`[otto] Web host ready.\n`)
        }
        consecutive5xx = 0
    ...
    if (now - lastTickAt >= TICKER_INTERVAL_MS) {
      lastTickAt = now
      if (hostUp) {
        stderr?.write(`[otto] Still waiting… (${elapsed()})\n`)
      } else {
        stderr?.write(`[otto] Waiting for web host… (${elapsed()})\n`)
      }
    }
...
function cleanupStaleInstance(cwd: string, stderr: WritableLike, registryPath?: string): void {
  ...
  stderr.write(`[otto] Cleaning up stale web server for ${key} (pid=${stale.pid}, port=${stale.port})…\n`)
  const result = killPid(stale.pid)
  if (result === 'killed') {
    stderr.write(`[otto] Killed stale web server (pid=${stale.pid}).\n`)
  } else if (result === 'already-dead') {
    stderr.write(`[otto] Stale web server was already stopped (pid=${stale.pid}) — clearing entry.\n`)
  } else {
    stderr.write(`[otto] Could not kill stale web server (pid=${stale.pid}): ${result.error}\n`)
  }
...
export async function launchWebMode(
  ...
  stderr.write(`[otto] Starting web mode…\n`)
  ...
  const env = {
    ...
    HOSTNAME: host,
    PORT: String(port),
    LOOP24_WEB_HOST: host,
    GSD_WEB_HOST: host,
    LOOP24_WEB_PORT: String(port),
    GSD_WEB_PORT: String(port),
    LOOP24_WEB_AUTH_TOKEN: authToken,
    GSD_WEB_AUTH_TOKEN: authToken,
    LOOP24_WEB_PROJECT_CWD: options.cwd,
    GSD_WEB_PROJECT_CWD: options.cwd,
    LOOP24_WEB_PROJECT_SESSIONS_DIR: options.projectSessionsDir,
    GSD_WEB_PROJECT_SESSIONS_DIR: options.projectSessionsDir,
    LOOP24_WEB_PACKAGE_ROOT: resolution.packageRoot,
    GSD_WEB_PACKAGE_ROOT: resolution.packageRoot,
    LOOP24_WEB_HOST_KIND: resolution.kind,
    GSD_WEB_HOST_KIND: resolution.kind,
    ...(resolution.kind === 'source-dev' ? { NEXT_PUBLIC_GSD_DEV: '1' } : {}),
    ...(options.allowedOrigins?.length ? { LOOP24_WEB_ALLOWED_ORIGINS: options.allowedOrigins.join(','), GSD_WEB_ALLOWED_ORIGINS: options.allowedOrigins.join(',') } : {}),
  }

  try {
    stderr.write(`[otto] Initialising resources…\n`)
  ...
  stderr.write(`[otto] Launching web host on port ${port}…\n`)
  ...
  try {
    ...
    try {
      ...
      stderr.write(`[otto] Could not open browser: ${browserError instanceof Error ? browserError.message : String(browserError)}\n`)
    }
  } catch (error) {
    const failure: WebModeLaunchFailure = {
      mode: 'web',
  ...
  stderr.write(`[otto] Ready → ${authenticatedUrl}\n`)

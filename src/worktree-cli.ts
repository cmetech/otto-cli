 ...
 * so nothing is lost. The workflow extension reads OTTO_CLI_WORKTREE to know
 ...
 * from resources/extensions/workflow/ which are shipped as raw .ts (#1283).
 */
...
function logDebugFailure(scope: string, error: unknown): void {
  if (process.env.OTTO_DEBUG === '1') {
    process.stderr.write(chalk.dim(`[otto] ${scope} failed: ${toErrorMessage(error)}\n`))
  }
}
...
async function handleWorktreeFlag(worktreeFlag: boolean | string): Promise<void> {
  ...
  if (worktreeFlag === true) {
    ...
    if (withChanges.length === 1) {
      ...
      process.env.OTTO_CLI_WORKTREE = wt.name
      process.env.OTTO_CLI_WORKTREE_BASE = basePath
  ...
  if (found) {
    ...
    process.env.OTTO_CLI_WORKTREE = name
    process.env.OTTO_CLI_WORKTREE_BASE = basePath
...
async function createAndEnter(ext: ExtensionModules, basePath: string, name: string): Promise<void> {
  try {
    ...
    if (hookError) {
      process.stderr.write(chalk.yellow(`[otto] ${hookError}\n`))
    }
    ...
    const msg = err instanceof Error ? err.message : String(err)
    process.stderr.write(chalk.red(`[otto] Failed to create worktree: ${msg}\n`))
    process.exit(1)
  }
}

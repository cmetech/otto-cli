/**
 * OTTO-2 MCP server real-process e2e (3-tool starter).
 *
 * Spawns the @otto-build/mcp-server CLI (`packages/mcp-server/dist/cli.js`)
 ...
 *   - otto_progress → structured project state read
 *
 * Note: this is the *orchestration* server (otto_doctor, otto_progress, …),
 * NOT `otto --mode mcp` (which exposes the agent's file-edit / bash tools
...
describe("mcp server e2e (real-process stdio)", () => {
	...
	test("connect, list tools, call otto_doctor + otto_progress against fresh project", { skip: skipReason ?? false, timeout: 60_000 }, async (t) => {
		...
		if (!sdk.ok) {
			...
			// Fresh project so the workflow tools have something deterministic to read.
			const project = createTmpProject({ git: true, ottoWorkflowSkeleton: true });
		t.after(project.cleanup);
		mkdirSync(join(project.dir, ".otto", "workflow", "milestones"), { recursive: true });
		...
		const transport = new sdk.TransportCtor({
			...
			env: {
				...
				OTTO_NON_INTERACTIVE: "1",
			},
			stderr: "pipe",
		});

			const client = new sdk.ClientCtor({ name: "otto-e2e-test", version: "0.0.0" });
		...
		for (const required of ["otto_doctor", "otto_progress", "otto_status"]) {
			assert.ok(toolNames.has(required), `tools/list missing ${required}. Got: ${[...toolNames].slice(0, 20).join(", ")}`);
		}

		// 2. otto_doctor — read-only structural health check.
		const doctor = await client.callTool({ name: "otto_doctor", arguments: { projectDir: project.dir }});
		assert.equal(doctor.isError, undefined, `otto_doctor returned error: ${JSON.stringify(doctor.content)}`);
		assert.ok(Array.isArray(doctor.content) && doctor.content.length > 0, "otto_doctor returned empty content");
		const doctorText = doctor.content
			.filter((c) => c.type === "text")
			.map((c) => c.text ?? "")
			.join("");
		assert.ok(doctorText.length > 0, "otto_doctor returned no text payload");

		// 3. otto_progress — structured project state read against the fresh project.
		const progress = await client.callTool({ name: "otto_progress", arguments: { projectDir: project.dir }});
		assert.equal(progress.isError, undefined, `otto_progress returned error: ${JSON.stringify(progress.content)}`);
		assert.ok(Array.isArray(progress.content) && progress.content.length > 0, "otto_progress returned empty content");
	});
});

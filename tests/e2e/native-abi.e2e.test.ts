/**
 * OTTO-2 native TS↔Rust ABI smoke.
 *
 * Loads `@otto/native` from a fresh node:test worker and exercises a few
 ...
 *
 * Skip path: if `@otto/native` cannot be resolved (e.g. running this file
...
async function tryLoadNative(): Promise<{ ok: true; mod: NativeShape } | { ok: false; reason: string }> {
	try {
		const mod = (await import("@otto/native")) as unknown as NativeShape;
		return { ok: true, mod };
	} catch (err) {
		return {
			ok: false,
			reason: `@otto/native not resolvable in this environment: ${(err as Error).message}`,
		};
	}
}
...
describe("native TS↔Rust ABI smoke", () => {
	test("xxHash32 round-trips matching the JS fallback for a known input", async (t) => {
		...
		if (typeof isNativeAddonLoaded === "function" && !isNativeAddonLoaded()) {
			t.skip("@cmetech/otto-engine-* optional dependency not installed — JS fallback only");
			return;
		}
	...
	test("searchContent finds matches in an in-memory buffer", async (t) => {
		...
		if (typeof searchContent !== "function") {
			t.skip("@otto/native.searchContent not exported in this build");
			return;
		}
		if (typeof isNativeAddonLoaded === "function" && !isNativeAddonLoaded()) {
			t.skip("@cmetech/otto-engine-* optional dependency not installed — JS fallback only");
